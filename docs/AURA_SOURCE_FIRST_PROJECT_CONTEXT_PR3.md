...
Optional candidates are ranked deterministically by declared relevance, then fixed category priority, then candidate ID. A candidate and its dependency closure are selected as a unit only when they fit. Candidate input edges must be a runtime `Sequence`; the compiler validates the sequence type and declared length before materialization or item traversal. They may be accepted up to a bounded four-times selected-edge budget (never above the module-wide edge ceiling) so task-conditioned endpoint reduction can occur before the final selected graph is checked strictly against `max_edges`. An edge incident to an omitted node is absent from the selected graph; the existing node omission bucket remains the attributable reason, rather than introducing a second edge-omission receipt plane.
