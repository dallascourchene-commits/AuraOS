...
`live_v1` is the exact result returned by the unchanged V1 writer. `shadow_v2` is a deterministic comparison record and cannot execute, authorize, rewrite recall, or alter the V1 packet.

## Exact-recall admission

A V2 `EXACT_RECALL` projection is accepted only after all of the following are verified:

1. The V1 decision is enabled and has a non-empty visible-ASCII compact candidate.
2. The caller capsule reproduces the V1 canonical original JSON and SHA-256 digest.
3. The persisted V1 record is retrievable by the emitted V1 pointer.
4. The record's pointer, dash key, glyph, holographic header, digest, content type, source hint, original bytes, and compressed packet are internally consistent.
5. The exact V1 original matches the caller capsule byte-for-byte.
6. The complete canonical V2 metadata suffix is included in the final unit count.
7. The V2 savings threshold still passes after that overhead.
8. The canonical V2 persistence receipt is logically bound to the already-verified V1 record; no duplicate record is written.
...
- V1 eligibility, reason, pointer, original digest, payload digest, exact-record identity, transmitted units, and measured savings;
...
- syntax compilation and Ruff correctness lint;
...
- pointer substitution and digest disagreement rejection;
- non-canonical metadata rejection;
- empty compact-output rejection;
- proof that canonical overhead can disable V2 without disabling or modifying V1;
- deterministic comparison digests and unchanged authority flags;
