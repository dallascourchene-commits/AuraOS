...
The public index must contain only normalized allowlisted source and documentation files. Its SHA-256 values and byte lengths must match the repository. Tests, workflows, caches, stores, logs, databases, environment files, runtime directories, CODEMAP, and topology are excluded.
