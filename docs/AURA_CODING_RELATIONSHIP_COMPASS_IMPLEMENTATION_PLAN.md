...
Merge **C0 plus this plan** first. Then rebase PR #175 onto that head and convert it into the C1–C3 sequence rather than attempting to finish every architectural layer in the existing draft PR.

The first new code after C0 should be `aura_relationship_contracts.py` and its schemas/tests. Locking the contracts before deeper integration prevents the reference's incompatible primitive definitions, regex authority, duplicate memory store, and premature patch capsules from becoming permanent API debt.
