...
def _exact_symbol(spec: Sequence[str]) -> tuple[str, str]:
    if (
        ...
        or len(spec) != 2
        or not all(isinstance(v, str) and v and v.strip() == v for v in spec)
    ):
...
def bind_generation(
    ...
    semantic_admission_surface_root: str,
    model_id: str,
    wrapper_symbol: Sequence[str] = _DEFAULT_WRAPPER_SYMBOL,
) -> GenerationBinding:
...
class GenerationBoundJsonSession:
    """Own the manifest-pinned wrapper/model in the child and expose JSON-only capabilities."""
    ...
    def __init__(
        ...
        *,
        ...
        load_args: Sequence[Any] = (),
        load_kwargs: Mapping[str, Any] | None = None,
    ) -> None:
        self._binding = bind_generation(
            subject_generation,
        ...
        _canonical_json(list(load_args))
        _canonical_json(kwargs)
    ...
    def status(self) -> dict[str, Any]:
        return {
            ...
            "wrapper_module": self._binding.wrapper_module,
            "wrapper_qualname": self._binding.wrapper_qualname,
...
def launch_generation_bound_json_service(
    *,
    model_id: str,
    model_path: str,
    model_allowlist: Mapping[str, Iterable[str]],
    ...
    proxy = IsolatedSessionProxy(
        ...
        factory_qualname="GenerationBoundJsonSession",
        allowed_methods=("status", "generate_json"),
    ...
    try:
        ...
        if (
            status.get("pid") != proxy.child_pid
            or status.get("schema") != _SCHEMA
            or status.get("subject_generation") != expected.subject_generation
            or status.get("semantic_admission_surface_root") != expected.semantic_admission_surface_root
            or status.get("model_id") != expected.model_id
            or status.get("wrapper_module") != expected.wrapper_module
            or status.get("wrapper_qualname") != expected.wrapper_qualname
