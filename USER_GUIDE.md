...
| `!empirical_lab <patch_repair|localizer|context|hotswap|research>` | You want to turn an Aura subsystem into a measurable experiment. | Defines a CODEMAP-grounded scorable task, creates a bounded candidate, scores the latest Live Architect transaction if one exists, records `Aura_Staging/empirical_candidate_tree.jsonl`, and only emits a promotion recommendation. It does not write production code. |
...
| `!stage` / `!stage_review` / `!review` | You need to inspect pending code before merge. | For live Architect transactions, prints status, selected council candidate, Judge path, hot-swap phase hash, topology-delta stats, staged patches, blockers, and failed Builder outputs/rejected diffs from `Aura_Staging/architect_live_transaction.json`. Legacy pending patches still show the older timestamp, target, confidence, and proposed-code preview. |
