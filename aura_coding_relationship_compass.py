...
def compile_coding_relationship_compass(
    ...
    *,
    ...
    route_reasons = _ordered_unique(
        [
            "explicit_relationship_compass_intent",
            *[f"matched_component:{component_id}" for component_id in matched_components],
            *[
                f"connectome_capability:{selection.capability_id}"
    ...
    packet: dict[str, Any] = {
        ...
        "connectome": {
            ...
            "capability_selections": [item.to_dict() for item in capability_selections],
            "capability_classes": capability_class_index(capability_selections),
            "route_reasons": list(objective_contract.route_reasons),
            "zero_model_eligible": objective_contract.zero_model_eligible,
        },
        "emergent_evidence": {
...
def relationship_compass_grounding(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(atlas, Mapping):
        atlas = {}
    objective_contract = packet.get("objective_contract") or {}
    if not isinstance(objective_contract, Mapping):
        objective_contract = {}
    connectome = packet.get("connectome") or {}
    ...
    return {
        ...
        "capability_path": list(connectome.get("required_capability_ids", []) or []),
        "capability_classes": dict(connectome.get("capability_classes", {}) or {}),
        "route_reasons": list(connectome.get("route_reasons", []) or []),
        "zero_model_eligible": bool(connectome.get("zero_model_eligible")),
        "objective_contract_digest": objective_contract.get("contract_id"),
