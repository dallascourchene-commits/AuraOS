...
IDENTITY_VERSION = "AURA_REFACTOR_STATE_IDENTITY_V1"
...
def normalize(value: Any) -> Any:
    ...
    if isinstance(value, Enum):
        return {"__enum__": f"{type(value).__module__}.{type(value).__qualname__}", "value": normalize(value.value)}
...
def history_events(session: Any) -> tuple[list[dict[str, Any]], str]:
    ...
    if explicit:
        return [{"sequence": index, "kind": str(item.get("kind") or "event") if isinstance(item, Mapping) else "event", "payload": normalize(item)} for index, item in enumerate(explicit)], "canonical_event_history"
    events: list[dict[str, Any]] = []
    for kind, values in (("turn", list(getattr(session, "turns", []) or [])), ("stage", list(getattr(session, "stage_results", []) or [])), ("verification", list(getattr(session, "verification_results", []) or []))):
        for item in values:
...
def history_identity(session_id: str, events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for index, event in enumerate(events):
        last = digest(event)
        root = digest({"session_id": session_id, "sequence": index, "previous_digest": root, "event_digest": last})
    return {"history_event_count": len(events), "history_root_digest": root, "last_event_digest": last, "last_sequence_number": len(events) - 1}


def semantic_sets(session: Any) -> dict[str, list[Any]]:
    aliases = {"assumptions": ("assumptions", "assumption_set"), "unresolved_questions": ("unresolved_questions", "open_questions"), "accepted_decisions": ("accepted_decisions", "decisions"), "rejected_alternatives": ("rejected_alternatives", "rejected_options")}
    ...
    for name, fields in aliases.items():
        ...
        for field in fields:
            ...
            if candidate is not None:
                value = candidate
                break
        result[name] = list(normalize(list(value or [])))
    return result
...
def build_projection(*, session: Any, tasks: list[dict[str, Any]], active_index: int, dependencies: dict[str, list[str]], repairs: dict[str, int], council_replan_count: int) -> dict[str, Any]:
    ...
    return {"session_id": str(getattr(session, "session_id", "") or ""), "plan_phase_hash": str(getattr(session, "plan_phase_hash", "") or ""), "active_task_index": active_index, "task_count": len(tasks), "completed_task_ids": completed, "current_task_id": current, "pending_role": str(getattr(getattr(session, "pending_turn", None), "role", "") or ""), "task_dependencies": dependencies, "dependency_frontier": frontier, "repair_attempts_by_task": repairs, "council_replan_count": council_replan_count, "execution_status": str(getattr(session, "status", "") or ""), **semantic_sets(session)}
...
def build_sidecar(session: Any, projection: dict[str, Any]) -> dict[str, Any]:
    ...
    body = {"identity_version": IDENTITY_VERSION, "sequence_mode": sequence_mode, "events": events, "projection": normalize(projection), **history}
...
def verify_sidecar(sidecar: Mapping[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    identity = history_identity(str(dict(sidecar.get("projection") or {}).get("session_id") or ""), events)
    if any(sidecar.get(key) != expected for key, expected in identity.items()):
        return False, {}
    return True, dict(sidecar.get("projection") or {})


__all__ = ["IDENTITY_VERSION", "build_projection", "build_sidecar", "canonical", "digest", "history_events", "history_identity", "normalize", "semantic_sets", "tokens", "verify_sidecar"]
