...
class MemoryStore:
    def __init__(self, filename, *, read_only=False):
        ...
        if self.read_only:
            path = str(filename)
            self.db = sqlite3.connect(f"file:{path}?mode=ro", uri=True, timeout=5, isolation_level=None)
        else:
        ...
        if version == 0 and self.read_only:
            self.db.close()
            raise ValueError('read-only memory store requires initialized schema')
        if version == 0:
    ...
    def _write(self):
        if self.read_only:
            raise MemoryConflict('read-only memory store forbids mutation')
