"""Deterministic semantic recipe for the 1,115-record K27 Memory City registry.

The recipe reproduces the original logical rows from the pinned cold-source
manifest and recovered 10x10x10 evaluation grammar. It does not authenticate
source currentness or grant authority.
"""
from __future__ import annotations
from dataclasses import dataclass, asdict
import hashlib

PRIMITIVES = ('K27_RECURSION',
 'MULTIPLEX_OVERLAY',
 'SPATIAL_WFST',
...
FAMILIES = {'K27_RECURSION': ('Bind each recursive path to an explicit frame generation and stable object ID',
 ...
 'MULTIPLEX_OVERLAY': ('Resolve overlay conflicts with explicit precedence and declared scope',
 ...
 'SPATIAL_WFST': ('Reject conflicting transition definitions in the spatial navigation graph',
 ...
 'PHYSICAL_JURISDICTION': ('Represent overlapping jurisdiction boundaries as typed independent claims',
 ...
 'XR_ANCHOR': ('Bind XR anchors to explicit tracking sessions and frame generations',
 ...
 'LOD_CULLING': ('Preserve record identity and coverage when scene detail levels change',
 ...
 'VISUAL_SKIN': ('Keep visual assets separate from the semantic city skeleton',
 ...
 'BREADBOARD': ('Keep experimental zones versioned and separate from published scene bindings',
 ...
 'XR_INTERACTION': ('Deduplicate interaction events with exact event and scene-generation IDs',
 ...
 'TENANCY_MULTIUSER': ('Resolve concurrent city edits using exact revision compare-and-swap',
...
METHODS = {'BLIND_ABLATION': ('paired baseline report',
                    'Compare the proposed component enabled and disabled on identical inputs, with results scored '
                    'before revealing the condition.'),
 'MUTATION': ('invariant regression suite',
              'Mutate one relevant source, generation, or context field at a time; assert the declared invariant or an '
              'explicit refusal.'),
 'FUZZ': ('seeded property campaign',
          'Generate bounded valid and invalid inputs, preserve failing seeds, and compare results to a simple '
          'exhaustive reference.'),
 'USER_STUDY': ('human evaluation protocol',
                'Predefine tasks and success criteria; obtain appropriate participant consent and record '
                'completion/error rates. Human evidence remains required.'),
 'HEADSET_BENCH': ('headset measurement protocol',
                   'Pin scene, device, runtime, workload and baseline; measure on a real headset. No physical '
                   'execution is inferred from simulation.'),
 'MOBILE_BENCH': ('mobile measurement protocol',
                  'Pin scene, device, runtime, workload and baseline; measure on a real mobile device. Report '
                  'correctness separately from latency.'),
 'CROSS_DEVICE_REPLAY': ('portable replay fixture',
                         'Replay the same versioned inputs on two declared runtimes/devices and compare semantic '
                         'outputs within declared numeric tolerances.'),
 'FAILURE_INJECTION': ('interruption recovery fixture',
                       'Interrupt the operation at persistence and publication boundaries; require either the complete '
                       'old state or the complete new state.'),
 'OWNER_CHALLENGE': ('owner integration review',
                     'Compare the change to the current owning implementation, document overlap, and require evidence '
                     'for every claimed residual improvement.'),
 'INTEGRATION_TEST': ('executable integration scenario',
                      'Exercise the proposed behavior through the actual public module APIs and assert observable '
                      'results, including an independent unaffected record.')}
...
MODULES = {'K27_RECURSION': 'k27_city.py / world_atlas.py / persistent_memory.py',
 ...
 'XR_ANCHOR': 'xr_scene.py / spatial_adapter.py',
 'LOD_CULLING': 'xr_scene.py / renderer',
 'VISUAL_SKIN': 'visual_skin.py',
 'BREADBOARD': 'spatial_adapter.py / BreadboardZone',
 'XR_INTERACTION': 'xr_jurisdiction.py / interaction owner',
 'TENANCY_MULTIUSER': 'tenant owner / persistent_memory.py local revision seam'}

SOURCES = {'COHERENCE': ['K27-EXT-001', 'K27-EXT-004'],
 'RETRIEVAL': ['K27-EXT-006', 'K27-EXT-007'],
 'CURRENTNESS': ['K27-EXT-003', 'K27-EXT-005'],
 'AUTHORITY': ['K27-EXT-001'],
 'PRIVACY': ['K27-EXT-005'],
 'PERFORMANCE': ['K27-EXT-010'],
 'ACCESSIBILITY': [],
 'RECOVERY': ['K27-EXT-002', 'K27-EXT-008'],
 'INTEROPERABILITY': ['K27-EXT-001'],
 'COGNITION_USABILITY': ['K27-EXT-009']}
...
def k27_for(i: int, j: int, k: int) -> tuple[int, int, int]:
    return (i * 2 % 27, j * 2 % 27, k * 2 % 27)


def generate_tickets() -> list[Ticket]:
    out = []
    for i, primitive in enumerate(PRIMITIVES):
        for j, concern in enumerate(CONCERNS):
            for k, operator in enumerate(OPERATORS):
                key = f"{primitive}|{concern}|{operator}"
                ticket_id = "MCXR-" + hashlib.sha256(key.encode()).hexdigest()[:10]
                out.append(Ticket(ticket_id, primitive, concern, operator, k27_for(i, j, k),
                    f"Can {primitive} survive {operator} pressure on {concern} without city labels becoming truth/authority?"))
    return out


def build_routes_and_families() -> tuple[list[dict], dict[str, dict]]:
    families: dict[str, dict] = {}
    routes: list[dict] = []
    for ticket in generate_tickets():
        family_id = ticket.primitive + "/" + ticket.concern
        change = FAMILIES[ticket.primitive][CONCERNS.index(ticket.concern)]
        evidence_keys = list(SOURCES[ticket.concern])
        if ticket.concern in ("AUTHORITY", "PRIVACY", "PERFORMANCE", "ACCESSIBILITY"):
            evidence_keys = []
        family = {
            "family_id": family_id, "primitive": ticket.primitive, "concern": ticket.concern,
            "proposed_change": change, "module_target": MODULES[ticket.primitive],
            "status": "candidate; full family not verified", "source_url": SOURCE,
            "external_source_keys": evidence_keys,
            "evidence_scope": "External sources motivate design only; they do not prove this Aura-specific feature.",
        }
        families[family_id] = family
        artifact, acceptance = METHODS[ticket.operator]
        score = 30 if ticket.primitive == "K27_RECURSION" else 15 if ticket.primitive == "MULTIPLEX_OVERLAY" else 5
        score += {"CURRENTNESS":20,"COHERENCE":15,"INTEROPERABILITY":15,"RECOVERY":15,"RETRIEVAL":10}.get(ticket.concern,0)
        score += {"INTEGRATION_TEST":20,"MUTATION":10,"FAILURE_INJECTION":15,"OWNER_CHALLENGE":5}.get(ticket.operator,-5)
        routes.append({**asdict(ticket), **family, "operator": ticket.operator, "original_question": ticket.question,
            "deliverable": f"{change}: {artifact}", "acceptance": acceptance, "priority_score": score,
            "provenance": "recovered O7 route, enriched with a proposed change; not a new discovery",
    ...
    routes.sort(key=lambda row: (-row["priority_score"], row["id"]))
