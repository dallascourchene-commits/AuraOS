...
class FrameTransform:
    ...
    def apply_digit(self, d: int) -> int:
        ...
        for j, src_axis in enumerate(self.axis_perm):
            v=xyz[src_axis]
            if self.invert[j]: v=2-v
            out.append(v)
        return self._encode(tuple(out))
    def apply(self, path: KPath) -> KPath:
        if not self.current: raise ValueError('stale transform')
        validate_path(path)
...
class FrameAtlas:
    ...
    def add_frame(self, frame: WorldFrame): self.frames[frame.frame_id] = frame
    def add_transform(self, t: FrameTransform): self.transforms[(t.src_frame, t.dst_frame)] = t
    def project(self, addr: FrameAddress, dst_frame_id: str) -> FrameAddress:
        src = self.frames.get(addr.frame_id); dst = self.frames.get(dst_frame_id)
        if not src or not dst or not src.current or not dst.current: raise ValueError('frame unavailable/stale')
        if src.generation != addr.frame_generation: raise ValueError('source address generation stale')
        if src.frame_id == dst.frame_id: return addr
        t = self.transforms.get((src.frame_id, dst.frame_id))
        if not t or not t.current: raise ValueError('current explicit transform required')
        if t.src_generation != src.generation or t.dst_generation != dst.generation: raise ValueError('transform generation mismatch')
...
class PrefixCoverage:
    def __init__(self, frame_id: str, frame_generation: str, prefixes: Iterable[KPath]=()):
        self.frame_id=frame_id; self.frame_generation=frame_generation; self.prefixes=frozenset(self._normalize(set(prefixes)))
    @staticmethod
    def _normalize(ps: set[KPath]) -> set[KPath]:
        ...
        for p in list(out):
            for k in range(len(p)):
                if p[:k] in out: out.discard(p); break
        changed=True
        while changed:
            changed=False
            ...
            for par in sorted(parents, key=len, reverse=True):
                ...
                if children <= out:
                    out.difference_update(children); out.add(par); changed=True
        return out
    def covers(self, addr: FrameAddress) -> bool:
        if addr.frame_id != self.frame_id or addr.frame_generation != self.frame_generation: return False
        return any(addr.path[:len(p)] == p for p in self.prefixes)
    def records(self) -> int: return len(self.prefixes)
