...
def digit_from_xyz(x:int,y:int,z:int)->int:
    if x not in TRITS or y not in TRITS or z not in TRITS:
        raise ValueError('trits must be 0..2')
    return x + 3*y + 9*z


def xyz_from_digit(d:int)->Tuple[int,int,int]:
    if not 0 <= d < 27: raise ValueError('digit must be 0..26')
...
class K27Path:
    ...
    def __post_init__(self):
        if any(not 0 <= d < 27 for d in self.digits): raise ValueError('bad K27 digit')
    ...
    def effective_rules(self,path:K27Path)->Dict[str,OverlayRule]:
        ...
        for cell in self.lineage(path):
            for k,r in cell.rules.items():
                prior=eff.get(k)
                if prior and prior.hard and not prior.delegable and prior.value != r.value:
