...
FROZEN=6e7c0f2806ca65b14f9c2100bff301b2c0336e6c
...
python ../control/.github/pr255_project_boundary_closure_patch.py
git diff --check
...
printf '%s\n' \
  ...
  | sort > /tmp/pr255-project-boundary-expected.txt
printf '%s\n' "${changed[@]}" > /tmp/pr255-project-boundary-actual.txt
diff -u /tmp/pr255-project-boundary-expected.txt /tmp/pr255-project-boundary-actual.txt
...
python -m pytest -q tests/test_aura_ephemeral_workspace_contracts.py | tee /tmp/pr255-project-boundary-pytest.txt
grep -Eq '^45 passed in ' /tmp/pr255-project-boundary-pytest.txt
python -m pytest --collect-only -q tests/test_aura_ephemeral_workspace_contracts.py | tee /tmp/pr255-project-boundary-collect.txt
grep -Eq '^45 tests collected in ' /tmp/pr255-project-boundary-collect.txt
...
test ! -e .github/workflows/pr255-project-boundary-closure.yml
test ! -e .github/pr255_project_boundary_closure_patch.py
test ! -e .github/pr255_project_boundary_closure_runner.sh
test -z "$(git status --porcelain=v1 --untracked-files=all | grep -E 'CODEMAP|topology_map|live_topology_ast|project-boundary-closure|project_boundary_closure' || true)"
...
git commit -m 'fix(pr1): reject project projection subclasses'
