"""Durable artifact persistence receipt + live index contract for CS-ARENA-SYNC-001 AS-06.

This is a pure coordination/evidence layer. It records verified durable artifact
persistence and projects a live index without becoming semantic ownership,
coordinate authority, effect authority, or execution proof.
"""
...
class ArtifactPersistenceReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        owner_status = _text(self.owner_binding_status, "OWNER_BINDING_STATUS_REQUIRED").upper()
        ...
        for field_name in ("producer_worker_id", "claim_id", "work_order_id", "observed_at"):
            object.__setattr__(
                self,
                field_name,
                _text(getattr(self, field_name), f"{field_name.upper()}_INVALID", allow_empty=True),
            )
        object.__setattr__(
            self, "prior_artifact_sid", _optional_text(self.prior_artifact_sid, "PRIOR_ARTIFACT_SID_INVALID")
        )
        object.__setattr__(
        ...
        if self.claim_id:
            if isinstance(self.claim_fence, bool) or not isinstance(self.claim_fence, int) or self.claim_fence < 1:
        ...
        if operation == "UPSERT":
            ...
            if isinstance(self.byte_size, bool) or not isinstance(self.byte_size, int) or self.byte_size < 0:
                raise ArtifactPersistenceError("BYTE_SIZE_INVALID")
        ...
        for field_name in (
            ...
            if _strict_bool(getattr(self, field_name), f"{field_name.upper()}_BOOLEAN_REQUIRED") is not False:
    ...
    def logical_payload(self) -> dict[str, Any]:
        # observed_at and receipt_id are intentionally excluded from consequence identity.
        return {
            "schema": self.schema,
...
def build_persistence_receipt_from_landed_verification(
    *,
    ...
    if event_type in {"DELETE", "TOMBSTONE"}:
        raise ArtifactPersistenceError("TOMBSTONE_REQUIRES_TOMBSTONE_RECEIPT")
    event_currentness = _text(event.get("source_currentness_ref"), "EVENT_CURRENTNESS_REQUIRED")
    currentness = _text(currentness_ref, "CURRENTNESS_REF_REQUIRED")
    ...
    if isinstance(byte_size, bool) or not isinstance(byte_size, int) or byte_size < 0:
        raise ArtifactPersistenceError("IDENTITY_BYTE_SIZE_INVALID")
    ...
    return ArtifactPersistenceReceipt(
        ...
        resource_ref=_text(verification.get("destination_resource_id"), "DESTINATION_RESOURCE_REQUIRED"),
        ...
        producer_worker_id=_text(
            event.get("producer_worker_id", ""), "PRODUCER_WORKER_ID_INVALID", allow_empty=True
        ),
        claim_id=_text(event.get("claim_id", ""), "CLAIM_ID_INVALID", allow_empty=True),
        claim_fence=claim_fence,
        work_order_id=_text(
            event.get("work_order_id", ""), "WORK_ORDER_ID_INVALID", allow_empty=True
        ),
...
def build_tombstone_receipt(
    *,
    ...
    if event.get("schema") != ARTIFACT_EVENT_SCHEMA:
        raise ArtifactPersistenceError("ARTIFACT_EVENT_SCHEMA_INVALID")
    event_type = _text(event.get("event_type"), "EVENT_TYPE_REQUIRED").upper()
    if event_type not in {"DELETE", "TOMBSTONE"}:
        raise ArtifactPersistenceError("TOMBSTONE_EVENT_REQUIRED")
    currentness = _text(currentness_ref, "CURRENTNESS_REF_REQUIRED")
    if _text(event.get("source_currentness_ref"), "EVENT_CURRENTNESS_REQUIRED") != currentness:
        raise ArtifactPersistenceError("STALE_CURRENTNESS_REBASE_REQUIRED")
...
def new_live_artifact_index(*, project_id: str, currentness_ref: str) -> dict[str, Any]:
    return {
        ...
        "currentness_ref": _text(currentness_ref, "CURRENTNESS_REF_REQUIRED"),
        "generation": 0,
        "receipts": {},
        "artifacts": {},
        "resource_heads": {},
...
def _normalize_index(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _text(out.get("currentness_ref"), "CURRENTNESS_REF_REQUIRED")
    generation = out.get("generation")
    if isinstance(generation, bool) or not isinstance(generation, int) or generation < 0:
        raise ArtifactPersistenceError("LIVE_INDEX_GENERATION_INVALID")
...
def index_revision(state: Mapping[str, Any]) -> str:
    ...
    # observation metadata may live in receipt rows, but logical revision keys only
    # durable consequence identities and current resource/artifact projection.
    logical = {
...
def _refresh_artifact_state(entry: dict[str, Any]) -> None:
    ...
    entry["state"] = "LIVE" if any(
        loc.get("state") == "LIVE" for loc in locations.values()
    ) else "TOMBSTONED"
...
def apply_persistence_receipt(
    ...
    *,
    ...
    row = receipt.to_dict()
    next_state["receipts"][receipt.receipt_id] = row
...
def artifact_available_event(
    ...
    *,
    ...
    event_type = "ARTIFACT_AVAILABLE" if receipt.operation == "UPSERT" else "ARTIFACT_TOMBSTONED"
    body = {
