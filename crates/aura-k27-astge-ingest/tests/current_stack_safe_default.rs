...
fn python_ast_reaches_current_generation_bound_readseek_safe_default() {
    ...
    assert!(
        encoded.pages.len() > 1,
        "fixture must exercise multiple physical pages"
    );
    ...
    for (expected_pbn, (pbn, page)) in encoded.pages.iter().enumerate() {
        assert_eq!(
            *pbn, expected_pbn as u64,
            "current generation uses absolute zero-based PBNs"
        );
    ...
    let admission = admit_data_serving_backend(&root, &node_path, &page_path, &binding, [0xD4; 32])
    ...
    assert!(
        observed.unique_pages > 1,
        "query must touch multiple S-plane pages"
    );
