use aura_k27_astge::{
    HydratedConeV1, NodeIndexRecordV1, PhysicalPageV1, StorageGenerationBindingV1, BLOCK_SIZE,
...
impl SnapshotManifestV1 {
    ...
    fn parse(bytes: &[u8]) -> Result<Self, MmapStorageError> {
        let text = std::str::from_utf8(bytes)
            .map_err(|_| MmapStorageError::InvalidManifest("utf8"))?;
        let mut fields =
            parse_fields(text).map_err(|_| MmapStorageError::InvalidManifest("field syntax"))?;
        ...
        let physical_locality_proven =
            match fields.remove("physical_locality_proven").as_deref() {
                Some("true") => true,
                Some("false") => false,
...
pub fn publish_generation(
    ...
    if final_dir.exists() {
        return Err(MmapStorageError::Io(
            "snapshot generation already exists".into(),
        ));
    }
...
impl ImmutableMmapReader {
    ...
    pub fn query_cone(
        ...
        while let Some((node_id, depth)) = queue.pop_front() {
            ...
            let record = self
                ...
                .get(&node_id)
                .cloned()
                .ok_or(MmapStorageError::MissingTarget(node_id))?;
    ...
    fn decode_page(&self, pbn: u64) -> Result<PhysicalPageV1, MmapStorageError> {
        ...
        let raw: [u8; BLOCK_SIZE] = self.pages_mmap[offset..end]
            .try_into()
            .map_err(|_| MmapStorageError::PageDecode)?;
...
fn validate_index_bytes(
    ...
    for raw in bytes.as_chunks::<NODE_INDEX_RECORD_SIZE>().0 {
        let record =
            NodeIndexRecordV1::decode(raw).map_err(|_| MmapStorageError::IndexRecordInvalid)?;
...
fn validate_page_bytes(
    ...
    for (pbn, chunk) in bytes.as_chunks::<BLOCK_SIZE>().0.iter().enumerate() {
        let page = PhysicalPageV1::decode(chunk).map_err(|_| MmapStorageError::PageDecode)?;
...
fn bind_page(
    ...
    if page.pbn != requested {
        return Err(MmapStorageError::PageNumberMismatch {
            requested,
            encoded: page.pbn,
        });
    }
...
fn parse_current(bytes: &[u8]) -> Result<CurrentPointer, MmapStorageError> {
    let text =
        std::str::from_utf8(bytes).map_err(|_| MmapStorageError::InvalidCurrent("utf8"))?;
    let mut fields =
        parse_fields(text).map_err(|_| MmapStorageError::InvalidCurrent("field syntax"))?;
    ...
    let generation_dir = fields
        ...
        .ok_or(MmapStorageError::InvalidCurrent("generation_dir"))?;
    ...
    let manifest_sha256 = fields
        ...
        .ok_or(MmapStorageError::InvalidCurrent("manifest digest"))?;
    if !valid_hash(&manifest_sha256) || !fields.is_empty() {
        return Err(MmapStorageError::InvalidCurrent(
...
fn parse_fields(text: &str) -> Result<BTreeMap<String, String>, ()> {
    ...
    for line in text.lines() {
        ...
        if key.is_empty()
            || value.is_empty()
            || out.insert(key.to_owned(), value.to_owned()).is_some()
        {
            return Err(());
        }
    }
...
fn take_u64(
    ...
    key: &'static str,
) -> Result<u64, MmapStorageError> {
    fields
        .remove(key)
        .ok_or(MmapStorageError::InvalidManifest(key))?
        .parse()
        .map_err(|_| MmapStorageError::InvalidManifest(key))
}
fn take_hash(
    fields: &mut BTreeMap<String, String>,
    key: &'static str,
) -> Result<String, MmapStorageError> {
    let value = fields
        .remove(key)
        .ok_or(MmapStorageError::InvalidManifest(key))?;
    if !valid_hash(&value) {
        return Err(MmapStorageError::InvalidManifest(key));
    }
    Ok(value)
}
fn take_digest32(
...
fn checked_len(count: u64, width: usize) -> Result<u64, MmapStorageError> {
    count
        ...
        .ok_or_else(|| MmapStorageError::Io("length overflow".into()))
}
...
fn hex(bytes: &[u8]) -> String {
    ...
    for byte in bytes {
        ...
        out.push(H[(byte & 0xf) as usize] as char);
    }
    out
}
fn verify_file(
    ...
    if !meta.is_file() {
        return Err(MmapStorageError::Io(format!(
    ...
    if sha256_file(path)? != digest {
        return Err(MmapStorageError::FileDigestMismatch(name));
    }
    Ok(())
}
...
#[cfg(not(unix))]
fn set_generation_read_only(_dir: &Path) -> Result<(), MmapStorageError> {
    Ok(())
}
#[cfg(unix)]
fn sync_dir(path: &Path) -> Result<(), MmapStorageError> {
    File::open(path)?.sync_all()?;
    Ok(())
}
#[cfg(not(unix))]
fn sync_dir(_path: &Path) -> Result<(), MmapStorageError> {
    Ok(())
}
...
mod tests {
    use super::*;
    use aura_k27_astge::{GenerationBoundGraphReader, PageRow};
    ...
    fn node(id: u64, pbn: u64, row: u16, degree: u16) -> NodeIndexRecordV1 {
        NodeIndexRecordV1 {
            node_id: id,
            semantic_handle_digest: d(id as u8 + 1),
            pbn,
            row,
            out_degree: degree,
            file_id: 1,
            byte_start: id as u32 * 10,
            byte_end: id as u32 * 10 + 5,
        }
    }
    fn page(
        pbn: u64,
        ...
        PhysicalPageV1 {
            ...
            placement_scheme_digest: d(scheme),
            rows,
            targets,
            edge_kinds: kinds,
        }
    ...
    fn fixture() -> (
        ...
        let binding = StorageGenerationBindingV1 {
            ...
            placement_scheme_digest: d(0x5A),
        };
        let records = vec![
            node(2, 1, 0, 1),
            node(0, 0, 0, 2),
    ...
    fn publication_mmap_and_readseek_are_query_equivalent() {
        ...
        let mmap = ImmutableMmapReader::open_current(&root).unwrap();
    ...
    fn successor_publication_does_not_retarget_already_open_reader() {
        ...
        assert_eq!(
            old.query_cone(0, 3, 16, None).unwrap(),
            new.query_cone(0, 3, 16, None).unwrap()
        );
        let _ = remove_dir_all(root);
    }
    ...
    fn page_or_index_tamper_after_publication_is_rejected_on_new_open() {
        ...
        assert_eq!(
            ImmutableMmapReader::open_current(&root).err(),
            Some(MmapStorageError::FileDigestMismatch(PAGE_FILE))
        );
        let _ = remove_dir_all(root);
    }
    ...
    fn current_manifest_digest_substitution_fails_closed() {
        ...
        let current = fs::read_to_string(&current_path)
            .unwrap()
            .replace("manifest_sha256=", "manifest_sha256=0");
        fs::write(&current_path, current).unwrap();
        assert!(matches!(
            ImmutableMmapReader::open_current(&root),
            Err(MmapStorageError::InvalidCurrent(_))
                | Err(MmapStorageError::ManifestDigestMismatch)
        ));
        let _ = remove_dir_all(root);
    }
    ...
    fn publication_rejects_generation_incoherent_page_bytes() {
        ...
        let (index_bytes, page_bytes) = bytes(&records, &pages);
        assert_eq!(
            publish_generation(&root, 8, binding, &index_bytes, &page_bytes).err(),
            Some(MmapStorageError::PlacementGenerationMismatch {
                expected: 11,
                observed: 12
            })
        );
