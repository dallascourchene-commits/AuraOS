...
def patch_compiler() -> None:
    ...
    start_marker = "    elif question.ambiguity_class == AmbiguityClass.CONTRADICTION.value:\n"
    end_marker = "    remaining = tuple("
...
def patch_gate() -> None:
    ...
    legacy_start_marker = "                elif question.candidate_answers:\n"
    legacy_end_marker = "                else:\n"
    legacy_start = text.index(legacy_start_marker)
    legacy_end = text.index(legacy_end_marker, legacy_start)
    current_legacy = text[legacy_start:legacy_end]
...
def patch_tests() -> None:
    ...
    if "def test_contradiction_clarification_preserves_the_selected_side" not in text:
        ...
        def test_contradiction_clarification_preserves_the_selected_side():
            from aura_bilateral_intent_compiler import analyze_bilateral_request, apply_clarification
            ...
            positive_question = next(
                item for item in positive_analysis.questions
            ...
            positive_resolved = apply_clarification(
                positive_analysis, question=positive_question, answer=positive_choice
            )
            ...
            negative_question = next(
                item for item in negative_analysis.questions
                if item.ambiguity_class == AmbiguityClass.CONTRADICTION.value
            )
            positive_choice, negative_choice = negative_question.candidate_answers
            negative_resolved = apply_clarification(
                negative_analysis, question=negative_question, answer=negative_choice
            )
    ...
    if rows:
        ...
        text = text.replace(marker, "".join(rows) + marker, 1)
    path.write_text(text, encoding="utf-8")
