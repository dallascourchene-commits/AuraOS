...
def _raise_original(caught: Exception, traceback: TracebackType | None) -> None:
...
def invoke_tool_shadow(
    ...
    *,
    ...
    actual_input_digest = stable_digest(sanitize_payload(kwargs))
    if actual_input_digest != decision.tool_input_digest:
        raise ValueError("decision tool input does not match tool_kwargs")
    ...
    try:
        ...
        if isinstance(value, Mapping):
            returned_status = str(value.get("status") or "").upper()
            if value.get("ok") is False or returned_status in {
                "FAILED",
                "DENIED",
                "ERROR",
            }:
                status = returned_status or "FAILED"
                error_class = str(
                    value.get("error_class") or value.get("error") or ""
                )
    except Exception as exc:  # preserve caller-visible exception type and traceback
        caught = exc
    ...
    try:
        ...
        if store is not None:
            decision_ref = store.store_payload(
                decision.to_dict(), kind="tool-decision", created_at=started
            )
            result_ref = store.store_payload(
                result.to_dict(), kind="tool-result", created_at=finished
            )
        ...
        decision_event = AuraEventEnvelope.create(
            ...
            proposal_only=True,
            measurement_classes={
                "confidence": decision.confidence_measurement_class
            },
        ...
        if caught is not None:
            _raise_original(caught, caught_traceback)
        raise
