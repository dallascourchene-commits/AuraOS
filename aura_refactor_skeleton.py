...
def _freeze(value: Any) -> Any:
    """Recursively freeze supported JSON-like values without key loss."""
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in sorted(value.items(), key=lambda pair: str(pair[0])):
            ...
            if normalized in frozen:
                raise ValueError(
                    f"mapping keys collide after string normalization: {normalized!r}"
                )
            frozen[normalized] = _freeze(item)
...
def _canonical_json(value: Any) -> bytes:
    return json.dumps(
        _thaw(_freeze(value)), sort_keys=True, separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
...
def _require_tuple(name: str, value: Any, item_type: Any) -> None:
    if not isinstance(value, tuple):
        raise ValueError(f"{name} must be an immutable tuple")
    if any(not isinstance(item, item_type) for item in value):
        raise ValueError(f"{name} contains an invalid item type")
...
class SourceSpan:
    ...
    def __post_init__(self) -> None:
        if _normalize_repo_path(self.path) != self.path:
    ...
    def create(cls, path: str, start_line: int, end_line: int) -> "SourceSpan":
        return cls(_normalize_repo_path(path), int(start_line), int(end_line))
...
class IntegrationDisposition:
    ...
    def create(
        ...
        return cls(str(structure).strip(), str(disposition).upper(), str(reason).strip())
...
class RefactorSkeletonNode:
    ...
    exact_source_hashes: Mapping[str, str] = field(
    ...
    def __post_init__(self) -> None:
        ...
        for name, value, item_type in (
            ("target_files", self.target_files, str),
            ("target_symbols", self.target_symbols, str),
            ("exact_source_spans", self.exact_source_spans, SourceSpan),
            ("dependencies", self.dependencies, str),
            ("invariants", self.invariants, str),
            ("acceptance_criteria", self.acceptance_criteria, str),
            ("required_tests", self.required_tests, str),
            ("risk_lanes", self.risk_lanes, str),
            ("repair_history", self.repair_history, Mapping),
            ("integration_dispositions", self.integration_dispositions, IntegrationDisposition),
        ):
            _require_tuple(name, value, item_type)
        ...
        if not self.node_digest or self.node_digest != _digest(_node_payload(self)):
    ...
    def _validate_ready_evidence_shape(self) -> None:
        ...
        if set(self.exact_source_hashes) != targets:
            raise ValueError(
                "READY_FOR_ACT requires exactly one source hash per target file"
            )
        invalid = [
            path for path, value in self.exact_source_hashes.items()
            if not _SHA256.fullmatch(str(value))
        ]
        if invalid:
            raise ValueError(f"invalid SHA-256 source hashes: {invalid}")
        span_paths = {item.path for item in self.exact_source_spans}
        missing = sorted(targets - span_paths)
        foreign = sorted(span_paths - targets)
    ...
    def create(
        ...
        *,
        ...
        for key, value in sorted(
            dict(exact_source_hashes or {}).items(), key=lambda pair: str(pair[0])
        ):
            if not str(key).strip() or not str(value).strip():
                continue
            normalized = _normalize_repo_path(key)
            if normalized in normalized_hashes:
                raise ValueError(
                    f"source hash keys collide after path normalization: {normalized}"
                )
            normalized_hashes[normalized] = str(value).strip().lower()
        fields = {
            ...
            "exact_source_hashes": MappingProxyType(normalized_hashes),
            "exact_source_spans": tuple(
            ...
            "integration_dispositions": tuple(
                item if isinstance(item, IntegrationDisposition)
    ...
    def validate_sources(self, repo_root: str | Path) -> list[str]:
        ...
        for relative_path in self.target_files:
            try:
                ...
                if sha256_file(source) != self.exact_source_hashes[relative_path]:
                    errors.append(
...
def _skeleton_identity(
    *, objective: str, domain: str, baseline_commit: str,
    source_plan_digest: str, addendum_digest: str,
) -> dict[str, str]:
    return {
        "objective": objective,
        "domain": domain,
...
class RefactorSkeleton:
    ...
    def __post_init__(self) -> None:
        ...
        known = set(node_ids)
        missing = sorted({
            dependency for node in self.nodes for dependency in node.dependencies
        ...
        if not self.skeleton_digest or self.skeleton_digest != _digest(_skeleton_payload(self)):
        ...
        *,
        ...
        fields = {
            ...
            **identity,
            ...
            "nodes": tuple(
                item if isinstance(item, RefactorSkeletonNode)
        ...
        *,
        ...
        revised = RefactorSkeletonNode.create(
            ...
            exact_source_hashes=(
                ...
                if exact_source_hashes is None else exact_source_hashes
            ),
            exact_source_spans=(
                current.exact_source_spans
                if exact_source_spans is None else exact_source_spans
            ),
            ...
            verifier_digest=(
                current.verifier_digest if verifier_digest is None else verifier_digest
            ),
    ...
    def validate(
        ...
        *,
        ...
        required = {str(item).strip() for item in required_structures if str(item).strip()}
        for node in self.nodes:
            if node.node_digest != _digest(_node_payload(node)):
                errors.append(f"{node.node_id}: node digest no longer matches content")
            missing = sorted(
                required - {item.structure for item in node.integration_dispositions}
            )
            if missing:
                errors.append(
...
class RefactorSkeletonStore:
    ...
    def _read_path(self, path: Path) -> RefactorSkeleton:
        ...
        if payload.get("envelope_digest") != self._envelope_digest(payload["skeleton"]):
