"""SQLite WAL ledger for authoritative ArenaExperience V2 records."""
...
ARENA_EXPERIENCE_LEDGER_VERSION="AURA_ARENA_EXPERIENCE_LEDGER_V2"
PATCH_AUTHORITY="exact_source_spans_and_hashes_only"; VSA_PATCH_AUTHORITY=False; _SCHEMA_VERSION=2
_SCHEMA="""
...
class ArenaExperienceLedger:
    def __init__(self,repo_root:str|Path=".",*,db_path:str|Path|None=None):
        root=Path(repo_root).resolve(); self.db_path=Path(db_path).resolve() if db_path else root/"Aura_Memory"/"arena_experience.db"; self.db_path.parent.mkdir(parents=True,exist_ok=True)
        self._conn=sqlite3.connect(str(self.db_path),timeout=10.); self._conn.row_factory=sqlite3.Row
        ...
        self._conn.executescript(_SCHEMA); self._migrate(); self._conn.commit()
    def _migrate(self):
        columns={str(r[1]) for r in self._conn.execute("PRAGMA table_info(arena_experiences)")}
        additions={"grammar_manifest_digest":"TEXT NOT NULL DEFAULT ''","outcome_vector_json":"TEXT NOT NULL DEFAULT '{}'","admissible_alternatives_json":"TEXT NOT NULL DEFAULT '[]'","predictions_json":"TEXT NOT NULL DEFAULT '[]'","route_observation_digest":"TEXT NOT NULL DEFAULT ''"}
        ...
        self._conn.execute("INSERT OR IGNORE INTO arena_experience_migrations(version,applied_at) VALUES (?,?)",(_SCHEMA_VERSION,time.time()))
    def record(self,experience:ArenaExperience|dict[str,Any]):
        ...
        raw["payload"],r0=sanitize_experience_payload(raw.get("payload") or {}); raw["admissible_alternatives"],r1=sanitize_experience_payload(raw.get("admissible_alternatives") or []); raw["predictions"],r2=sanitize_experience_payload(raw.get("predictions") or [])
        raw["redactions"]=sorted(set([str(x) for x in raw.get("redactions",[])]+r0+r1+r2)); raw.setdefault("version",ARENA_EXPERIENCE_VERSION)
        required=("experience_id","correlation_id","arena_id","arena_version","grammar_version","grammar_manifest_digest","runtime_version","compiler_version","state_before","state_after","final_outcome","outcome_vector")
        missing=[k for k in required if not _present(raw.get(k))]
        ...
        values=(eid,str(raw.get("correlation_id") or ""),str(raw.get("task_id") or ""),str(raw.get("workflow_id") or ""),str(raw.get("arena_id") or ""),str(raw.get("arena_version") or ""),str(raw.get("grammar_version") or ""),str(raw.get("grammar_manifest_digest") or ""),str(raw.get("runtime_version") or ""),str(raw.get("compiler_version") or ""),started,completed,str(raw.get("state_before") or ""),str(raw.get("state_after") or ""),str(raw.get("selected_transition") or ""),str(raw.get("final_outcome") or ""),_j(raw.get("outcome_vector") or {}),_j(raw.get("admissible_alternatives") or []),_j(raw.get("predictions") or []),str(raw.get("route_observation_digest") or ""),str(raw.get("repository_commit_sha") or ""),str(raw.get("working_tree_digest") or ""),str(raw.get("objective_hash") or ""),str(raw.get("source_hash_digest") or ""),str(raw.get("provider") or ""),str(raw.get("model") or ""),str(raw.get("measurement_class") or "UNAVAILABLE"),str(raw.get("cost_run_id") or ""),_j(raw.get("trace_atom_ids") or []),_j(raw.get("raw_evidence_refs") or []),_j(raw.get("redactions") or []),_j(raw.get("payload") or {}),digest,str(raw.get("version") or ARENA_EXPERIENCE_VERSION),time.time())
        try:
            self._conn.execute("""INSERT INTO arena_experiences(experience_id,correlation_id,task_id,workflow_id,arena_id,arena_version,grammar_version,grammar_manifest_digest,runtime_version,compiler_version,started_at,completed_at,state_before,state_after,selected_transition,final_outcome,outcome_vector_json,admissible_alternatives_json,predictions_json,route_observation_digest,repository_commit_sha,working_tree_digest,objective_hash,source_hash_digest,provider,model,measurement_class,cost_run_id,trace_atom_ids_json,raw_evidence_refs_json,redactions_json,payload_json,experience_digest,schema_version,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",values); self._conn.commit()
    ...
    def history(self,*,arena_id:str="",task_id:str="",grammar_manifest_digest:str="",limit:int=50):
        clauses=[]; params=[]
        ...
        params.append(max(1,min(int(limit),10000))); where=f"WHERE {' AND '.join(clauses)}" if clauses else ""
        return [_decode(r) for r in self._conn.execute(f"SELECT * FROM arena_experiences {where} ORDER BY completed_at DESC LIMIT ?",params).fetchall()]
    def export_jsonl(self,path,*,arena_id:str="",limit:int=10000):
        ...
        return {"ok":True,"path":str(out),"record_count":len(rows),"patch_authority":PATCH_AUTHORITY,"vsa_patch_authority":False}
    def status(self):
        count=int(self._conn.execute("SELECT COUNT(*) FROM arena_experiences").fetchone()[0]); complete=int(self._conn.execute("SELECT COUNT(*) FROM arena_experiences WHERE grammar_manifest_digest!='' AND outcome_vector_json!='{}'").fetchone()[0])
        return {"ok":True,"version":ARENA_EXPERIENCE_LEDGER_VERSION,"schema_version":_SCHEMA_VERSION,"db_path":str(self.db_path),"journal_mode":str(self._conn.execute("PRAGMA journal_mode").fetchone()[0]).lower(),"record_count":count,"v2_complete_record_count":complete,"legacy_record_count":count-complete,"patch_authority":PATCH_AUTHORITY,"vsa_patch_authority":False}
    def close(self):self._conn.close()
    def __enter__(self):return self
    def __exit__(self,*_):self.close()

def _decode(row):
    data=dict(row); defaults={"trace_atom_ids_json":[],"raw_evidence_refs_json":[],"redactions_json":[],"payload_json":{},"outcome_vector_json":{},"admissible_alternatives_json":[],"predictions_json":[]}
