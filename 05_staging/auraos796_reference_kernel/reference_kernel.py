...
class OwnerRef:
    ...
    def validate(self) -> None:
        ...
        if self.object_kind not in {"issue", "pull"}:
            raise ValueError("OBJECT_KIND_REQUIRED")
        if self.ordinal <= 0:
...
class JSpaceStore:
    ...
    def append(self, event: JSpaceEvent) -> tuple[str, int]:
        ...
        try:
            ...
            if row:
                if tuple(row[1:]) != semantic:
                    raise ValueError("CONFLICTING_EVENT_REPLAY")
                conn.execute("COMMIT")
                return "DUPLICATE_COLLAPSED", row[0]
            cur = conn.execute(
    ...
    def add_dep(self, src: str, dst: str, kind: str = "CONSEQUENCE") -> None:
        conn = self.connect()
        conn.execute("INSERT OR IGNORE INTO deps(src,dst,kind) VALUES(?,?,?)", (src, dst, kind))
        conn.close()

    def affected_cone(self, changed: Iterable[str]) -> set[str]:
        conn = self.connect()
        adjacency: dict[str, set[str]] = {}
        for src, dst, _ in conn.execute("SELECT src,dst,kind FROM deps"):
    ...
    def events(self) -> list[tuple]:
        conn = self.connect()
        rows = conn.execute(
            "SELECT seq,event_id,event_type,jid,visit_id,source_ref,source_generation,owner_epoch,subject,payload_json,recorded_at_ns FROM events ORDER BY seq"
        ).fetchall()
        conn.close()
        return rows
    ...
    def project(self) -> dict[int, dict]:
        ...
        applied: dict[tuple[int, str, str], tuple[int, str]] = {}
        for seq, _, event_type, jid, visit, source, generation, _, subject, payload_json, _ in self.events():
            state = projection.setdefault(jid, {
                "jid": jid, "visit_id": visit, "status": "UNKNOWN", "objective": None, "last_seq": 0,
                "sources": {}, "authority_effect": False, "runtime_liveness": "UNKNOWN", "source_sequences": {},
            })
            payload = json.loads(payload_json)
            stream = (jid, source, subject)
            source_sequence = payload.get("source_sequence")
            if source_sequence is not None:
                prior_sequence, prior_generation = applied.get(stream, (-1, ""))
                if source_sequence < prior_sequence:
                    state["last_seq"] = max(state["last_seq"], seq)
                    continue
                if source_sequence == prior_sequence:
                    if generation != prior_generation:
                        raise ValueError("SOURCE_SEQUENCE_CONFLICT")
                    state["last_seq"] = max(state["last_seq"], seq)
                    continue
                applied[stream] = (source_sequence, generation)
                state["source_sequences"][f"{source}|{subject}"] = source_sequence
            state["visit_id"] = visit
            state["last_seq"] = max(state["last_seq"], seq)
            state["sources"][source] = generation
            ...
            state["runtime_liveness"] = "UNKNOWN"
        return projection
...
class WakeReducer:
    ...
    def decide(self, changed: set[str], capability_nodes: set[str], deterministically_closed: set[str], all_nodes: set[str]) -> FrontierDecision:
        if not changed:
            return FrontierDecision("NOOP", (), (), tuple(sorted(all_nodes)), "NO_MATERIAL_DELTA")
        cone = self.store.affected_cone(changed)
        unresolved = cone - deterministically_closed
        hydrate = tuple(sorted(x for x in unresolved if x in capability_nodes or x in changed))
        cold = tuple(sorted(all_nodes - cone))
        if not unresolved:
            return FrontierDecision("CLOSED_DETERMINISTIC", tuple(sorted(cone)), (), cold, "ALL_AFFECTED_CLOSED")
...
class EvidenceLeaseGate:
    ...
    def compare(self, observed: Mapping[str, EvidenceLease], all_nodes: set[str]) -> LeaseDecision:
        ...
        if not changed:
            return LeaseDecision("CURRENT_PLANNING_CANDIDATE", (), (), tuple(sorted(all_nodes)))
        cone = self.store.affected_cone(changed)
...
class ReconcileEngine:
    def __init__(self, store: JSpaceStore, *, jid: int, visit_id: str, owner_epoch: str):
        self.store, self.jid, self.visit_id, self.owner_epoch = store, jid, visit_id, owner_epoch

    def _projected_streams(self) -> dict[str, tuple[str, int | None]]:
        projected = self.store.project().get(self.jid, {})
        sources = projected.get("sources", {})
        sequences = projected.get("source_sequences", {})
        return {source: (generation, sequences.get(f"{source}|{source}")) for source, generation in sources.items()}
    ...
    def reconcile(self, snapshots: Iterable[SourceState], *, all_nodes: set[str], capability_nodes: set[str], deterministic_closed: set[str] = frozenset()) -> ReconcileReceipt:
        prior = self._projected_streams()
        changed: list[str] = []
        appended: list[str] = []
        for snapshot in sorted(snapshots, key=lambda item: item.source_ref):
            if not snapshot.source_ref or not snapshot.generation:
                raise ValueError("SOURCE_BINDING_REQUIRED")
            prior_generation, prior_sequence = prior.get(snapshot.source_ref, ("", None))
            if snapshot.source_sequence is not None and prior_sequence is not None:
                if snapshot.source_sequence < prior_sequence: continue
                if snapshot.source_sequence == prior_sequence:
                    if snapshot.generation != prior_generation: raise ValueError("SOURCE_SEQUENCE_CONFLICT")
                    continue
            elif prior_generation == snapshot.generation and snapshot.current:
                continue
            changed.append(snapshot.source_ref)
            payload = {"observed_generation": snapshot.generation, "current": snapshot.current}
            ...
            event_id = "reconcile:" + hashlib.sha256(f"{self.jid}|{snapshot.source_ref}|{snapshot.generation}|{snapshot.current}|{snapshot.source_sequence}".encode()).hexdigest()
        ...
        disposition = "NOOP_SOURCE_SNAPSHOT" if not changed else ("RECONCILED_CLOSED" if decision.disposition == "CLOSED_DETERMINISTIC" else "RECONCILED_RESIDUAL")
        body = {"disposition": disposition, "changed_sources": sorted(changed), "appended_events": sorted(appended), "affected": decision.affected, "hydrate": decision.hydrate, "cold_preserved": decision.cold_preserved}
        return ReconcileReceipt(disposition, tuple(sorted(changed)), tuple(sorted(appended)), decision.affected, decision.hydrate, decision.cold_preserved, digest(body))
...
class GitHubObservationNormalizer:
    def workflow(self, observation: WorkflowObservation) -> dict:
        observation.owner.validate()
        semantic_failure = False
        pre_job = False
        admission = "UNKNOWN"
        if observation.status == "completed" and observation.conclusion == "action_required" and observation.job_count == 0:
            pre_job, admission = True, "PRE_JOB_ACTION_REQUIRED"
        elif observation.job_count > 0 and observation.conclusion == "failure":
            semantic_failure, admission = True, "JOBS_EXECUTED_FAILURE"
...
class JoinContext:
    protocol_root: str
    intent_root: str
    current_branch_head: str | None
    active_residual: str | None
    affected_neighborhood: tuple[str, ...]
    current_sources: tuple[tuple[str, str], ...]
    next_obligation: str | None
    packet_digest: str
    effect_authority: bool = False
...
class JoinContextCompiler:
    def compile(self, *, store: JSpaceStore, jid: int, protocol_root: str, intent_root: str, current_branch_head: str | None, active_residual: str | None, affected: Iterable[str], required_sources: Iterable[str], next_obligation: str | None, max_affected: int = 25) -> JoinContext:
        ...
        sources = store.project().get(jid, {}).get("sources", {})
        current = tuple(sorted((source, sources[source]) for source in set(required_sources) if source in sources))
