...
def main() -> int:
    ...
    if count != 1:
        raise RuntimeError(f"CODEMAP scanner compatibility anchor expected once, found {count}")
    TARGET.write_text(text.replace(OLD, NEW, 1), encoding="utf-8")
    print("Phase-3 CODEMAP scanner positional compatibility repair applied")
    return 0
