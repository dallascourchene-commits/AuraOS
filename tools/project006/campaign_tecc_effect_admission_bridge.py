...
class I: command_id:str='cmd'; idempotency_key:str='idem'; source_digest:str=r('src')
@dataclass(frozen=True)
...
def run(cases=24000):
    c={'cases':cases,'oracle_bind':0,'oracle_nonbind':0,'candidate_false_bind':0,'candidate_false_hold':0,'legacy_unproofed_false_ready':0,'effect_ready':0}
    for z in range(cases):
        mode=z%20; args=mutate(mode); expected=mode in (0,19)
    ...
    raw=json.dumps(c,sort_keys=True,separators=(',',':')).encode(); out={**c,'campaign_root':sha256(raw).hexdigest(),'schema':'aura.mc_o14.tecc_effect_admission.campaign.v1','authority':D0}
