...
3. require direct binding construction to prove that the supplied terminal contribution is a member of the canonical required contribution set;
4. require direct operation construction to receive the exact terminal-attempt facts, rederive the expected G6 binding identities from those protected relations, and reject missing/extra/substituted caller binding-ID sets;
5. derive one immutable G6 attempt/result binding per exact required contribution, with `acceptance_operation_digest` **absent** from the binding schema and identity preimage;
6. make public `validate_g6_binding_record` semantic: it requires independently resolved `AcceptanceFacts` plus the exact `AttemptTerminalFacts`, reconstructs the expected record, and canonical-byte-compares it; the self-hash-only checker is private/internal and is not an authority path;
7. canonicalize the exact G6 binding-identity set;
8. derive the closed G6 acceptance-operation body and digest downstream;
9. on restart, independently rebuild accepted-result identity, binding records/identities, binding set and operation digest in that order, then compare to stored state;
10. reject generation integers outside the interoperable RFC8785/ECMAScript safe-integer range `0..9007199254740991`, avoiding cross-validator JCS numeric divergence;
11. fail closed on missing/extra/substituted required relations, caller-chosen operation relation sets, rehashed protected-fact transplantation, noncanonical sets, profile aliasing, identity/digest transplantation, malformed protected facts, non-COMPLETE lifecycle, unsafe generation integers, or stored/recomputed mismatch.
...
The reference now contains **28 unittest regression methods** across `test_workercrystal_acceptance_g6.py` and `test_workercrystal_jcs_safe_integer.py`, covering:
...
- direct binding rejection for terminals outside the canonical required relation set;
- direct operation construction requiring exact protected terminal facts;
- direct operation rejection for missing, extra and substituted caller binding-ID sets and terminal sets outside required relations;
...
- G6 operation binding-set permutation invariance when the supplied set is semantically exact;
...
J163/V02 authored the original repository generation and must not independently review or certify it. The current semantic-validator and required-relation-set repairs are also constructor-authored work and must receive fresh Different-J review. That reviewer must bind the exact PR head, execute/inspect the tests and attack the G6 schema, protected-fact crossbinding, direct-builder required-relation membership, direct-operation exact binding-set ownership, identity direction, canonicalization including the JCS safe-integer ceiling, restart/transplant behavior, version aliases and the claim ceiling before any adoption consequence.