...
import json
HARD=('producer_exact','consumer_exact_independent','verifier_authorization_exact','semantic_generation_current','full_source_operation_identity_exact','journal_schema_and_owner_current','durable_admission_exact','authority_separation')
def hard_decision(a):
...
def run():
    ...
    p={'schema':'aura.mc_o14r.omega8.v2','hard_axes':HARD,**d,'effect_ready':0,'authority':'D0_NONPROMOTING'}; p['root']=sha256(json.dumps(p,sort_keys=True,separators=(',',':')).encode()).hexdigest()
