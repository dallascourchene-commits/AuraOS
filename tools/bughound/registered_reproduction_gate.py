"""Source-owned independent-reproduction gate for BugHound cash candidates.

This is the PR442 trust membrane: callers cannot provide the reproduction
registry, a lookup callback, expected reproduction identities, or a trusted
record. The production registry is intentionally empty until a real,
independently observed reproduction producer is pinned by source change.
"""
...
def independent_reproduction_registry_receipt() -> BugHoundIndependentReproductionRegistryReceiptV2:
    records = tuple(sorted(_CANONICAL_REPRODUCTION_RECORDS, key=lambda record: record.record_digest))
    return BugHoundIndependentReproductionRegistryReceiptV2(
        ...
        active_record_count=sum(
            ...
            if record.registry_current and record.independently_observed and not record.revoked and not record.external_effect
        ),
    )
