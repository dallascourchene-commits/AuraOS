...
def main() -> int:
    ...
    if count != 1:
        raise RuntimeError(f"_iter_repo_files compatibility anchor expected once, found {count}")
    TARGET.write_text(text.replace(OLD, NEW, 1), encoding="utf-8")
    Path(__file__).unlink()
    print("_iter_repo_files retains historical positional skip_dirs compatibility")
    return 0
