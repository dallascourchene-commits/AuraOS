...
class PersistentKVReuseTargetV1:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(
            self, "coordinate_ref", _text(self.coordinate_ref, "COORDINATE_REF_INVALID")
        )
        if (
            isinstance(self.k27_cell, bool)
            or not isinstance(self.k27_cell, int)
            or not 0 <= self.k27_cell < 27
        ):
        ...
        for f in (
            ...
            "system_tool_prefix_digest",
            "prefix_token_digest",
            "principal_namespace_digest",
            "workload_digest",
        ):
...
class PersistentKVPathEvidenceV1:
    ...
    def __post_init__(self):
        ...
        for f in (
            ...
            "queue_penalty_us",
            "memory_penalty_us",
            "security_isolation_us",
            "invalidation_penalty_us",
        ):
    ...
    def net_reuse_us(self):
        return self.prefill_saved_us - sum(
            (
                self.transfer_us,
                self.restore_us,
                self.queue_penalty_us,
                self.memory_penalty_us,
                self.security_isolation_us,
                self.invalidation_penalty_us,
            )
        )


def resolved_projection_payload_digest(
    target: PersistentKVReuseTargetV1, path: PersistentKVPathEvidenceV1
) -> str:
    if not isinstance(target, PersistentKVReuseTargetV1) or not isinstance(
        path, PersistentKVPathEvidenceV1
    ):
        raise KVAdmissionError("KV_TARGET_AND_PATH_REQUIRED")
    return _dig(
        "AURA_PCK2_RESOLVED_PROJECTION_V1",
        {"target_digest": target.target_digest, "path_digest": path.path_digest},
    )
...
class OwnerResolverProofV1:
    ...
    def __post_init__(self):
        ...
        for f in (
            ...
            object.__setattr__(self, f, _text(getattr(self, f), f"{f.upper()}_INVALID"))
        for f in (
            "projection_claim_digest",
            "owner_head",
            "owner_blob",
            "owner_recognized_projection_digest",
            "resolver_signature",
        ):
            object.__setattr__(self, f, _sha(getattr(self, f), f"{f.upper()}_INVALID"))
        if self.supersedes_proof_digest is not None:
            object.__setattr__(
                ...
                _sha(self.supersedes_proof_digest, "SUPERSEDES_PROOF_DIGEST_INVALID"),
            )
...
def build_resolver_proof(
    *,
    ...
    u = {
        ...
        "owner_head": claim.owner_head,
        "owner_blob": claim.owner_blob,
        "owner_abi": claim.owner_abi,
        "resolver_ref": _text(resolver_ref, "RESOLVER_REF_INVALID"),
        "resolver_generation": _text(resolver_generation, "RESOLVER_GENERATION_INVALID"),
        "resolver_currentness_ref": _text(
            resolver_currentness_ref, "RESOLVER_CURRENTNESS_REF_INVALID"
        ),
        "source_ref": claim.source_ref,
        "source_generation": claim.source_generation,
        "source_currentness_ref": claim.source_currentness_ref,
        "owner_recognized_projection_digest": _sha(
            owner_recognized_projection_digest,
        ...
        "schema": "OwnerResolverProofV1",
    }
    sig = _sign(u, key)
    return OwnerResolverProofV1(
        **{**u, "disposition": disposition, "resolver_signature": sig}
    )
...
def _current_proof_digests(
    proof_state: Mapping[str, Any], claim_digest: str
) -> tuple[str, ...]:
...
def _verify_resolver(
    claim: KVReuseProjectionClaimV1,
    proof: OwnerResolverProofV1,
    keys: Mapping[str, Any],
    state: Mapping[str, Any],
    proof_state: Mapping[str, Any],
):
    if not isinstance(claim, KVReuseProjectionClaimV1) or not isinstance(
        proof, OwnerResolverProofV1
    ):
    ...
    if not hmac.compare_digest(
        proof.resolver_signature,
        _sign(proof.signing_payload(), keys[proof.resolver_ref]),
    ):
        raise KVAdmissionError("RESOLVER_SIGNATURE_INVALID")
    current = _current_proof_digests(proof_state, claim.claim_digest)
    if proof.proof_digest not in current:
...
def admit_persistent_kv_reuse(
    *,
    ...
    logical = {
        ...
        "cache_read_observed": path_evidence.cache_read_observed,
        "cache_hit_tokens": path_evidence.cache_hit_tokens,
        "prefill_saved_us": path_evidence.prefill_saved_us,
        "transfer_us": path_evidence.transfer_us,
        "restore_us": path_evidence.restore_us,
        "queue_penalty_us": path_evidence.queue_penalty_us,
        "memory_penalty_us": path_evidence.memory_penalty_us,
        "security_isolation_us": path_evidence.security_isolation_us,
        ...
        "coordinate_memory_equated_to_transformer_kv": False,
        "semantic_response_cache_equated_to_transformer_kv": False,
        "monetary_credit_authorized": False,
    ...
    return {
        **logical,
        "admission_digest": _dig("AURA_PCK2_ADMISSION_V1", logical),
    }
