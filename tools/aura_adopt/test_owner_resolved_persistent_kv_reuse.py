...
def target(**kw):
    data=dict(
        ...
        prefix_token_digest=D, cache_abi="kvabi:v3", principal_namespace_digest=E,
...
def path(t=None, **kw):
    ...
    data=dict(
        ...
        principal_namespace_digest=t.principal_namespace_digest, host_epoch=t.host_epoch,
        ...
        memory_penalty_us=250, invalidation_penalty_us=250,
    )
    data.update(kw)
...
def admit_raw(t,c,p,e,keys=None,state=None):
...
class PCK2Tests(unittest.TestCase):
    def test_positive_control(self):
        ...
        self.assertEqual(7000,out["net_reuse_us"])
    ...
    def test_model_mismatch_blocks_even_when_owner_resolved(self):
        out=admit(model_revision="model:other")
        self.assertIn("PATH_MODEL_REVISION_MISMATCH",out["blockers"])
    ...
    def test_zero_net_benefit_not_admissible(self):
        out=admit(prefill_saved_us=3000,transfer_us=1000,restore_us=1000,queue_penalty_us=500,memory_penalty_us=250,invalidation_penalty_us=250)
