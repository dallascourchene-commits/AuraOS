...
AXES=(m.SOURCE,m.PLACEMENT,m.OWNER,m.RESOLVER,m.TARGET,m.RUNTIME,m.PRINCIPAL)
def issuers():
    return {a:(f'issuer:{a.lower()}','issuer-gen:1') for a in AXES}
...
class Resolver:
    ...
    def resolve_choice_generation(self,q):
        ...
        return m.ChoiceGenerationResolutionV1(
            s.get('query_digest',q.query_digest),
            s.get('evidence_domain',q.evidence_domain),
            s.get('issuer_ref',q.issuer_ref),
            s.get('issuer_generation',q.issuer_generation),
            s.get('resolver_ref','resolver:aura'),
            s.get('resolver_generation','resolver-gen:1'),
            s.get('resolver_currentness_ref','resolver-current:1'),
            s.get('proof_ref','proof:'+q.axis.lower()),
            s.get('currentness_state','CURRENT'),
            s.get('revoked',False))
...
class T(unittest.TestCase):
    ...
    def test_target_stale_only_blocks_one_choice(self):
        ...
        class R(Resolver):
            def resolve_choice_generation(self,q):
                ...
                return m.ChoiceGenerationResolutionV1(q.query_digest,q.evidence_domain,q.issuer_ref,q.issuer_generation,'resolver:aura','resolver-gen:1','resolver-current:1','proof:'+q.axis.lower(),state,False)
    ...
    def test_cross_choice_resolution_replay(self):
        a=choice('route:a');ra=req(a);qa=m._query(a,ra,ra.by_axis()[m.TARGET]);replay=m.ChoiceGenerationResolutionV1(qa.query_digest,qa.evidence_domain,qa.issuer_ref,qa.issuer_generation,'resolver:aura','resolver-gen:1','resolver-current:1','proof:target','CURRENT',False)
    ...
    def test_remote_runtime_provider_binding(self):
        c=choice('route:r',True,'provider:b');r=req(c);axes=list(r.axes);i=next(i for i,x in enumerate(axes) if x.axis==m.RUNTIME);old=axes[i];axes[i]=m.GenerationAxisRequirementV1(old.axis,old.evidence_domain,old.issuer_ref,old.issuer_generation,'provider:wrong',old.generation,old.currentness_ref,old.authoritative_basis_ref);bad=m.ChoiceGenerationRequirementV1(c.choice_digest,tuple(axes))
    ...
    def test_principal_binding(self):
        c=choice();r=req(c);axes=list(r.axes);i=next(i for i,x in enumerate(axes) if x.axis==m.PRINCIPAL);old=axes[i];axes[i]=m.GenerationAxisRequirementV1(old.axis,old.evidence_domain,old.issuer_ref,old.issuer_generation,'principal:b',old.generation,old.currentness_ref,old.authoritative_basis_ref);bad=m.ChoiceGenerationRequirementV1(c.choice_digest,tuple(axes))
    ...
    def test_authority_flags_false(self):
        ...
        for k in ('domain_authority_proven_by_this_module','resolver_trust_proven_by_this_module','credential_authorized','model_download_authorized','provider_call_authorized','payment_authorized','network_authorized','effect_authorized','execution_proven'):self.assertFalse(d[k])
    def test_wrong_evidence_domain_blocks_target(self):
        c=choice();d=m.compile_presentable_choice(c,req(c),resolver=Resolver({m.TARGET:{'evidence_domain':m.DOMAIN_BY_AXIS[m.OWNER]}}));self.assertIn('EVIDENCE_DOMAIN_MISMATCH:TARGET',d['blockers'])
    def test_wrong_issuer_generation_blocks_target(self):
        c=choice();d=m.compile_presentable_choice(c,req(c),resolver=Resolver({m.TARGET:{'issuer_generation':'issuer-gen:old'}}));self.assertIn('ISSUER_GENERATION_MISMATCH:TARGET',d['blockers'])
    def test_missing_issuer_binding_rejected(self):
        c=choice();ii=issuers();ii.pop(m.OWNER)
        with self.assertRaisesRegex(m.ChoiceMembraneError,'GENERATION_ISSUER_BINDING_REQUIRED:OWNER'):
            m.build_choice_generation_requirement(c,issuer_by_axis=ii,resolver_ref='resolver:aura',resolver_generation='g',resolver_currentness_ref='c',source_currentness_ref='s',owner_currentness_ref='o',target_currentness_ref='t',principal_currentness_ref='p',source_basis_ref='bs',owner_basis_ref='bo',resolver_basis_ref='br',target_basis_ref='bt',principal_basis_ref='bp')
    def test_requirement_domain_cross_cast_rejected(self):
        with self.assertRaisesRegex(m.ChoiceMembraneError,'GENERATION_EVIDENCE_DOMAIN_MISMATCH:TARGET'):
            m.GenerationAxisRequirementV1(m.TARGET,m.DOMAIN_BY_AXIS[m.OWNER],'issuer:target','issuer-gen:1','target:x','gen:1','cur:1','basis:target')
