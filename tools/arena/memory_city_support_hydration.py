...
class SupportClosedHydration:
    status:str; support_cut:tuple[str,...]; missing_objects:tuple[str,...]; selected_bytes:int; universe_bytes:int; hydration_fraction:float; plan_root:str; receipt_root:str; authority_minted:bool=False; gate10:bool=False
def compile_support_closed_hydration(direct_objects:Iterable[str],belief_states:Iterable[SupportBeliefState],locator_by_object:Mapping[str,SourceSpanLocator],*,max_hydration_bytes:int)->SupportClosedHydration:
    ...
    if cut.status!='READY_INVARIANT_ROUTE_CUT_D0':return _out('HOLD_ROUTE_SUPPORT',cut.safe_support_cut,(),0,universe,cut.cut_root)
    missing=tuple(sorted(set(cut.safe_support_cut)-set(locator_by_object)))
    if missing:return _out('HOLD_SPAN_MAP_GAP',cut.safe_support_cut,missing,0,universe,cut.cut_root)
    ...
    return _out(status,cut.safe_support_cut,(),plan.selected_bytes,universe,plan.receipt_root)
def _out(status,cut,missing,selected,universe,plan_root):
    fraction=selected/universe if universe else 0.0; payload={'status':status,'support_cut':tuple(cut),'missing':tuple(missing),'selected':selected,'universe':universe,'plan_root':plan_root}
    return SupportClosedHydration(status,tuple(cut),tuple(missing),selected,universe,fraction,plan_root,digest(payload))
