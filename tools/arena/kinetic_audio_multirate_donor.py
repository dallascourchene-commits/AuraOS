...
def classify_currentness_lattice(axes:Sequence[int])->str:
    if len(axes) not in (8,13) or any(type(v) is not int or v not in (0,1,2) for v in axes):raise ValueError("lattice axes must be 8 or 13 trits")
    return "HOLD_HARD_INVALID" if 0 in axes else "HOLD_UNKNOWN" if 1 in axes else "READY"
...
class RawGestureIntent:
    ...
    def verify(self):return self.raw_digest==digest([self.gesture,self.magnitude_milli,self.k27.code,self.frame_index])
...
class ProjectedMusicalIntent:
    kind:ControlKind;controls:tuple[tuple[str,int],...];k27:K27Coordinate;source_gesture:str;source_magnitude_milli:int;source_frame_index:int;source_digest:str
    ...
    def __post_init__(self):
        ...
        if len({k for k,_ in self.controls})!=len(self.controls) or not isinstance(self.k27,K27Coordinate) or not self.source_gesture or not _sha(self.source_digest) or not _sha(self.basis_projection_root):raise ValueError("invalid identity")
    def source_verified(self):return type(self.source_magnitude_milli) is int and 0<=self.source_magnitude_milli<=1000 and type(self.source_frame_index) is int and 0<=self.source_frame_index<=2**63-1 and self.source_digest==digest([self.source_gesture,self.source_magnitude_milli,self.k27.code,self.source_frame_index])
    @property
    def intent_digest(self):return digest([self.kind,list(self.controls),self.k27.code,self.source_gesture,self.source_magnitude_milli,self.source_frame_index,self.source_digest,self.basis_graph_generation,self.basis_mutation_epoch,self.basis_timeline_epoch,self.basis_projection_root,self.authority])

@dataclass(frozen=True)
class OwnerSnapshot:
    graph_generation:int;mutation_epoch:int;timeline_epoch:int;graph_root:str;projection_root:str;sample_cursor:int;performance_revision:int;bpm_milli:int;scale_index:int;drop_state:int;soft_controls:tuple[tuple[str,int],...];phrase_origin_sample:int;state_root:str
...
class K27ConstraintProjector:
    ...
    def project(self,r,s):
        if not isinstance(r,RawGestureIntent) or not r.verify():raise ValueError("invalid raw gesture provenance")
        if r.gesture in self.SOFT:
            c=r.k27.centered;idx={"open":0,"pinch":1,"spread":2}[r.gesture];name={"open":"brightness_milli","pinch":"density_milli","spread":"width_milli"}[r.gesture];controls=((name,self.clamp(500+170*c[idx]+(r.magnitude_milli-500)//3,0,1000)),);kind="soft"
            ...
            if r.gesture=="fist":controls=(("drop_state",int(r.magnitude_milli>=500)),)
                ...
                d=1 if r.gesture=="swipe_up" else -1;c=r.k27.centered;step=4000+1000*abs(c[0])+500*abs(c[1]);controls=(("bpm_milli",self.clamp(s.bpm_milli+d*step,60000,180000)),)
        else:raise ValueError("unsupported gesture")
        return ProjectedMusicalIntent(kind,controls,r.k27,r.gesture,r.magnitude_milli,r.frame_index,r.raw_digest,s.graph_generation,s.mutation_epoch,s.timeline_epoch,s.projection_root)

@dataclass(frozen=True)
class EpochBoundPermit:
    graph_generation:int;mutation_epoch:int;timeline_epoch:int;graph_root:str;projection_root:str;intent_digest:str;issued_sample:int;target_sample:int;permit_digest:str
    @classmethod
    def build(cls,s,i,t):
        b=[s.graph_generation,s.mutation_epoch,s.timeline_epoch,s.graph_root,s.projection_root,i.intent_digest,s.sample_cursor,t];return cls(*b,digest(b))
    def verify(self):return self.permit_digest==digest([self.graph_generation,self.mutation_epoch,self.timeline_epoch,self.graph_root,self.projection_root,self.intent_digest,self.issued_sample,self.target_sample])

@dataclass(frozen=True)
class ScheduledEvent:event_id:str;admission_seq:int;intent:ProjectedMusicalIntent;permit:EpochBoundPermit;ramp_frames:int
@dataclass(frozen=True)
...
class FixedEventQueue:
    ...
    def push(self,e):
        ...
        p=self.n;k=(e.permit.target_sample,e.admission_seq)
        while p and (self.s[p-1].permit.target_sample,self.s[p-1].admission_seq)>k:self.s[p]=self.s[p-1];p-=1
    ...
    def drop_if(self,pred):
        ...
        for i in range(self.n):
            e=self.s[i]
            if pred(e):d.append(e.event_id)
            else:self.s[w]=e;w+=1
        for i in range(w,self.n):self.s[i]=None
        self.n=w;return tuple(d)
...
class KineticAudioOwner:
    ...
    def __init__(self,*,recipe_root,engine_root,sample_rate=48000,block_size=128,bpm_milli=128000,beats_per_bar=4,bars_per_phrase=4,queue_capacity=128,lookahead_blocks=2,smoothing_ms=20):
        ...
        self.sample_rate=_bounded(sample_rate,8000,384000,"sample_rate");self.block_size=_bounded(block_size,16,8192,"block_size");self.beats_per_bar=_bounded(beats_per_bar,1,32,"beats_per_bar");self.bars_per_phrase=_bounded(bars_per_phrase,1,128,"bars_per_phrase");self.lookahead_blocks=_bounded(lookahead_blocks,0,1024,"lookahead");self.smoothing_ms=_bounded(smoothing_ms,0,10000,"smoothing");_bounded(bpm_milli,20000,400000,"bpm")
        self.rr=recipe_root;self.er=engine_root;self.g=self.m=self.t=1;self.pr=self.cur=0;self.bpm=bpm_milli;self.scale=self.drop=self.phrase_origin=self.next_admission_seq=0;self.soft={"brightness_milli":500,"density_milli":500,"width_milli":500};self.q=FixedEventQueue(queue_capacity)
    @property
    ...
    def _proj(self):return digest([self.g,self.m,self.t,self.graph_root,self.pr,self.bpm,self.scale,self.drop,self.phrase_origin,sorted(self.soft.items())])
    def _state(self):return digest([self._proj(),self.cur,self.q.ids(),self.next_admission_seq])
    def snapshot(self):return OwnerSnapshot(self.g,self.m,self.t,self.graph_root,self._proj(),self.cur,self.pr,self.bpm,self.scale,self.drop,tuple(sorted(self.soft.items())),self.phrase_origin,self._state())
    def _current(self,p):return p.verify() and (p.graph_generation,p.mutation_epoch,p.timeline_epoch,p.graph_root,p.projection_root)==(self.g,self.m,self.t,self.graph_root,self._proj())
    def governed_graph_write(self,*,recipe_root=None,engine_root=None):
        ...
        changed=(nr,ne)!=(self.rr,self.er);self.rr,self.er=nr,ne;self.g+=int(changed);self.m+=1;inv=self.q.drop_if(lambda e:not self._current(e.permit));s=self.snapshot();return GraphWriteReceipt(changed,inv,s,digest([changed,list(inv),s.state_root]))
    def _validate(self,i,s):
        if not isinstance(i,ProjectedMusicalIntent):return "HOLD_INVALID_INTENT"
        if not i.source_verified():return "HOLD_SOURCE_PROVENANCE"
        ...
        for k,v in i.controls:
            if type(v) is not int:return "HOLD_CONTROL_SCHEMA"
        ...
        return None
    def _phrase_frames(self):return Fraction(self.sample_rate*60*1000*self.beats_per_bar*self.bars_per_phrase,self.bpm)
    def _phrase_target(self,minsample):
        ph=self._phrase_frames();delta=max(0,minsample-self.phrase_origin);n=_ceil(Fraction(delta,1)/ph);return max(minsample,self.phrase_origin+_ceil(ph*n))
    def schedule(self,i):
        ...
        p=EpochBoundPermit.build(s,i,target);seq=self.next_admission_seq;eid=digest([p.permit_digest,seq,list(i.controls),i.k27.code,ramp]);e=ScheduledEvent(eid,seq,i,p,ramp)
        if not self.q.push(e):return ScheduleReceipt(False,"HOLD_QUEUE_FULL",None,None,len(self.q))
        self.next_admission_seq+=1;return ScheduleReceipt(True,"ADMIT_SCHEDULED",eid,target,len(self.q))
    def _apply(self,e):
        if e.intent.kind=="soft":self.soft.update(dict(e.intent.controls))
        else:
            for k,v in e.intent.controls:
                if k=="bpm_milli":self.phrase_origin=e.permit.target_sample;self.bpm=v
...
class KineticAudioMembrane:
    ...
    def ingest(self,o):
        r=self.compiler.observe(o);return None if r is None else self.owner.schedule(self.projector.project(r,self.owner.snapshot()))

@dataclass(frozen=True)
class SyntheticBenchmark:
    blocks:int;p50_ns:int;p95_ns:int;p99_ns:int;max_ns:int;callback_budget_ns:int;overruns:int;control_path_blocks:int=0;claim_ceiling:str="PROCESS_LEVEL_SYNTHETIC_ONLY"

def synthetic_callback_benchmark(owner,blocks=20000,*,control_every_blocks=0):
    _bounded(blocks,100,1000000,"blocks")
    if type(control_every_blocks) is not int or control_every_blocks<0:raise ValueError("control_every_blocks must be non-negative int")
    xs=[];cp=0;p=K27ConstraintProjector();frame=0
    for bi in range(blocks):
        if control_every_blocks and bi%control_every_blocks==0 and owner.queue_depth==0:
            raw=RawGestureIntent.build(GestureObservation(frame,"open",900,650,K27Coordinate(1,1,1)));frame+=1;r=owner.schedule(p.project(raw,owner.snapshot()));cp+=int(r.admitted)
        t=time.perf_counter_ns();owner.process_block();xs.append(time.perf_counter_ns()-t)
    xs.sort();pct=lambda q:xs[min(len(xs)-1,max(0,math.ceil(q*len(xs))-1))];budget=int(owner.block_size/owner.sample_rate*1e9);return SyntheticBenchmark(blocks,int(statistics.median(xs)),pct(.95),pct(.99),xs[-1],budget,sum(x>budget for x in xs),cp)
