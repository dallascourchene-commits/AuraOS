...
def evidence_fixture(g, a, o):
    aw = dict(a.accepted_witness_roots); pp = dict(a.proof_projection_roots); sd = dict(a.semantic_domain_roots); vg = dict(a.verifier_generations)
    evidence = {}
    # topological construction so dependency roots can bind existing outputs/domains.
    for node_id in g.topo_order:
        n = g.by_id[node_id]
        provisional = EvidenceWitness(
            node_id=node_id, graph_root=g.graph_root, witness_root=aw[node_id], output_root=D(("out", node_id)),
            verifier_generation=vg[n.verifier_id], dependency_input_root=R("0"), projection_root=pp[node_id], semantic_domain_root=sd[node_id]
        )
        evidence[node_id] = provisional
        evidence[node_id] = replace(provisional, dependency_input_root=dependency_input_root(g, node_id, evidence))
    return evidence


class T(unittest.TestCase):
    def setUp(self):
        self.g = graph_fixture(); self.a, self.o = surfaces(self.g); self.e = evidence_fixture(self.g, self.a, self.o)
    ...
    def test_clean_surface_reuses_everything(self):
        p = compile_reproof_plan(self.g, explicit_changed_roots=(), evidence=self.e, admission=self.a, current_owner=self.o)
        self.assertEqual(p.recompute_order, ())
        self.assertEqual(set(p.reuse_nodes), set(self.g.by_id))
        self.assertEqual(p.authority_ceiling, "D0_EXTERNAL_AUTH_UNPROVEN")

    def test_numeric_domain_drift_seeds_exact_descendant_cone(self):
        sd = dict(self.o.semantic_domain_roots); sd["NUMERIC_SOURCE"] = R("c")
        o = CurrentOwnerSurface.mint_identity_surface(graph_root=self.g.graph_root, verifier_generations=self.o.verifier_generations,
            projection_roots=self.o.projection_roots, semantic_domain_roots=sd.items(), owner_replay_receipt_root=self.o.owner_replay_receipt_root)
        p = compile_reproof_plan(self.g, explicit_changed_roots=(), evidence=self.e, admission=self.a, current_owner=o)
        self.assertEqual(p.drift_seeds, ("NUMERIC_SOURCE",))
        self.assertEqual(set(p.recompute_order), {"NUMERIC_SOURCE","COST_RECEIPT","EFFICIENCY_REUSE","FINAL_REUSE"})
        self.assertIn("SECURITY_RECEIPT", p.reuse_nodes)

    def test_projection_drift_is_not_silently_reused(self):
        pp = dict(self.o.projection_roots); pp["COST_RECEIPT"] = R("d")
        o = CurrentOwnerSurface.mint_identity_surface(graph_root=self.g.graph_root, verifier_generations=self.o.verifier_generations,
            projection_roots=pp.items(), semantic_domain_roots=self.o.semantic_domain_roots, owner_replay_receipt_root=self.o.owner_replay_receipt_root)
        p = compile_reproof_plan(self.g, explicit_changed_roots=(), evidence=self.e, admission=self.a, current_owner=o)
        self.assertEqual(set(p.recompute_order), {"COST_RECEIPT","EFFICIENCY_REUSE","FINAL_REUSE"})

    def test_generation_drift_wakes_all_owned_nodes_then_descendants(self):
        vg = dict(self.o.verifier_generations); vg["V_COST"] = "g-new"
        o = CurrentOwnerSurface.mint_identity_surface(graph_root=self.g.graph_root, verifier_generations=vg.items(),
            projection_roots=self.o.projection_roots, semantic_domain_roots=self.o.semantic_domain_roots, owner_replay_receipt_root=self.o.owner_replay_receipt_root)
        p = compile_reproof_plan(self.g, explicit_changed_roots=(), evidence=self.e, admission=self.a, current_owner=o)
        self.assertTrue({"WORKLOAD_COST","COST_RECEIPT","EFFICIENCY_REUSE","FINAL_REUSE"}.issubset(p.recompute_order))

    def test_trace_change_reopens_only_reuse_lane(self):
        p = compile_reproof_plan(self.g, explicit_changed_roots=("TRACE",), evidence=self.e, admission=self.a, current_owner=self.o)
    ...
    def test_security_change_does_not_reopen_cost_branch(self):
        p = compile_reproof_plan(self.g, explicit_changed_roots=("SECURITY_SOURCE",), evidence=self.e, admission=self.a, current_owner=self.o)
        self.assertEqual(set(p.recompute_order), {"SECURITY_SOURCE","SECURITY_RECEIPT","FINAL_REUSE"})
        self.assertIn("COST_RECEIPT", p.reuse_nodes)

    def test_unadmitted_reused_witness_fails(self):
        e = dict(self.e); e["SECURITY_RECEIPT"] = replace(e["SECURITY_RECEIPT"], witness_root=R("e"))
        with self.assertRaisesRegex(ReproofContractError, "UNADMITTED_WITNESS"):
            compile_reproof_plan(self.g, explicit_changed_roots=("NUMERIC_SOURCE",), evidence=e, admission=self.a, current_owner=self.o)

    def test_dependency_detachment_fails(self):
        e = dict(self.e); e["SECURITY_RECEIPT"] = replace(e["SECURITY_RECEIPT"], dependency_input_root=R("f"))
        with self.assertRaisesRegex(ReproofContractError, "DEPENDENCY_DETACHMENT"):
            compile_reproof_plan(self.g, explicit_changed_roots=("NUMERIC_SOURCE",), evidence=e, admission=self.a, current_owner=self.o)

    def test_authority_widening_fails(self):
        e = dict(self.e); e["SECURITY_RECEIPT"] = replace(e["SECURITY_RECEIPT"], effect_authority=True)
    ...
    def test_cross_graph_surface_fails(self):
        ...
        with self.assertRaisesRegex(ReproofContractError, "CROSS_GRAPH_SURFACE"):
            compile_reproof_plan(self.g, explicit_changed_roots=(), evidence=self.e, admission=self.a, current_owner=o)
    ...
    def test_unknown_changed_root_fails(self):
        with self.assertRaisesRegex(ReproofContractError, "UNKNOWN_CHANGED_ROOT"):
            compile_reproof_plan(self.g, explicit_changed_roots=("NOPE",), evidence=self.e, admission=self.a, current_owner=self.o)
