...
def classify(i,k):
    ...
    if k==9:
        return LeaseRegistrySnapshot(1,()),[ResourceProposal(p,())],ResourceDisposition.READY
    raise ValueError(k)
def run(n=100000):
    mismatch=0; false_ready=0; owner_cas_false=0; lease_cas_false=0; roots=[]
    owner=OwnerSnapshot(H,T)
    for i in range(n):
        k=i%10; reg,ps,exp=classify(i,k); q=plan_resource_absorption(owner,reg,ps,now_s=NOW)
        if q.disposition!=exp: mismatch+=1
        if exp is not ResourceDisposition.READY and q.disposition is ResourceDisposition.READY: false_ready+=1
        if i<1000: roots.append(q.manifest_root)
    for i in range(1000):
        reg,ps,exp=classify(i,0); q=plan_resource_absorption(owner,reg,ps,now_s=NOW)
        r1=commit_resource_absorption(q,observed_owner_head='9'*64,observed_lease_root=reg.root)
        r2=commit_resource_absorption(q,observed_owner_head=H,observed_lease_root=LeaseRegistrySnapshot(2,reg.leases).root)
        if r1.committed or r1.write_count: owner_cas_false+=1
        if r2.committed or r2.write_count: lease_cas_false+=1
    ...
    out={'cases':n,'mismatches':mismatch,'false_ready':false_ready,'owner_cas_false':owner_cas_false,'lease_cas_false':lease_cas_false,'hs1000':1000,'omega8_keepers':omega,'omega8_states':6561,'13d_repairs':repairs,'13d_tails':243,'sample_root':digest(roots)}
    out['campaign_root']=digest(out)
    print(json.dumps(out,sort_keys=True))
    assert mismatch==false_ready==owner_cas_false==lease_cas_false==repairs==0 and omega==1
