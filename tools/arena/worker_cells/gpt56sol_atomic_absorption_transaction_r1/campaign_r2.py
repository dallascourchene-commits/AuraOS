...
LINE=hashlib.sha256(b'line').hexdigest()
...
def oracle(kind):
    return {
      0:Disposition.READY,1:Disposition.READY,2:Disposition.CONFLICT_HOLD,3:Disposition.CONFLICT_HOLD,
      4:Disposition.CONFLICT_HOLD,5:Disposition.REBASE_REQUIRED,6:Disposition.DEBRIS_HOLD,7:Disposition.AUTHORITY_HOLD,
      8:Disposition.CONFLICT_HOLD,9:Disposition.READY}[kind]

def case(i,kind):
    c=hx(f'cons-{i//10}'); r=hx(f'rec-{i//10}'); blob=hx(f'blob-{i}'); files={f'pkg/f{i%7}.py':blob}
    a=P(f'A{i}',c,r,files)
    if kind==0: return [a]
...
def run(n=100000):
    mismatches=0; false_ready=0; exact_redelivery_fail=0; roots=[]
    for i in range(n):
        k=i%10; pl=plan(SNAP,case(i,k)); exp=oracle(k)
        if pl.disposition!=exp: mismatches+=1
        if k in (2,3,4,5,6,7,8) and pl.disposition==Disposition.READY: false_ready+=1
        if k==1 and (pl.disposition!=Disposition.READY or not pl.collapsed_proposals): exact_redelivery_fail+=1
        if i<1000: roots.append(pl.manifest_root)
    hs_mismatch=sum(plan(SNAP,case(i,i%10)).disposition!=oracle(i%10) for i in range(1000))
    om=sum(omega8_keeper(x) for x in itertools.product(range(3),repeat=8))
    bad=(2,2,2,2,2,2,1,1); tails=sum(context13_preserves_invalid(bad,t) for t in itertools.product(range(3),repeat=5))
    bridge_files={
    ...
    bp=plan(SNAP,[bridge]); br=commit(bp,H)
    out={'random_cases':n,'mismatches':mismatches,'false_ready':false_ready,'exact_redelivery_fail':exact_redelivery_fail,
         'hs1000_cases':1000,'hs1000_mismatches':hs_mismatch,'omega8_states':6561,'omega8_keepers':om,'tail13_states':243,'invalid_repairs':tails,
         'bridge_fixture_disposition':bp.disposition.value,'bridge_fixture_write_count':len(bp.writes),'bridge_fixture_commit':br.committed,
         'bridge_fixture_manifest_root':bp.manifest_root,'sample_manifest_root':digest(roots),'campaign_root':''}
