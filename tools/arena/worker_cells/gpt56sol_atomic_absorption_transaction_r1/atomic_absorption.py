...
def _text(x,n):
    if type(x) is not str or not x or any(ord(c)<32 for c in x): raise E(n)
    return x

def _hex(x,n):
    _text(x,n)
    if HEX64.fullmatch(x) is None: raise E(n)
    return x

def _path(p):
    _text(p,'BAD_PATH')
    if p.startswith('/') or '..' in p.split('/'): raise E('BAD_PATH')
    return p
...
def validate_proposal(p:Proposal):
    ...
    if type(p.asks_effect_authority) is not bool: raise E('BAD_AUTHORITY_FLAG')
    if not p.files: raise E('EMPTY_PROPOSAL')
    for path,blob in p.files.items(): _path(path); _hex(blob,'BAD_BLOB_DIGEST')

def plan(snapshot:OwnerSnapshot, proposals:Sequence[Proposal])->PublicationPlan:
    _hex(snapshot.head,'BAD_OWNER_HEAD'); _hex(snapshot.tree_root,'BAD_TREE_ROOT')
    if not proposals: raise E('NO_PROPOSALS')
    ...
    for p in accepted:
        for path,blob in sorted(p.files.items()):
            if path not in path_owner:
                path_owner[path]=(p.proposal_id,blob); writes[path]=blob
            else:
...
def commit(plan:PublicationPlan, observed_head:str)->PublicationReceipt:
    ...
    if plan.disposition!=Disposition.READY:
        return PublicationReceipt(False,observed_head,None,digest(asdict(plan)),plan.manifest_root,0,0)
    if observed_head!=plan.expected_head:
        return PublicationReceipt(False,observed_head,None,digest(asdict(plan)),plan.manifest_root,0,len(plan.accepted_proposals))
    new_head=digest({'parent':observed_head,'manifest':plan.manifest_root,'writes':list(plan.writes)})
    return PublicationReceipt(True,observed_head,new_head,digest(asdict(plan)),plan.manifest_root,len(plan.writes),0)
