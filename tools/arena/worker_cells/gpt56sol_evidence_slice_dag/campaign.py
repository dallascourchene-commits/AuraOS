...
def main():
    d=demo_dag(); w=demo_witnesses(d); rng=random.Random(SEED)
    false=0; recompute=0; all_nodes=len(d.nodes); h=hashlib.sha256()
    start=time.perf_counter()
    for i in range(100000):
        changed=set(rng.sample(list(d.nodes), 1 if i%4 else 2))
        p=d.compile_plan(changed,w); want=closure_oracle(d,changed)
    ...
    for i in range(1000):
        ...
        p=d.compile_plan(changed,w)
        hs_false += set(p.invalidated)!=closure_oracle(d,changed)
    out={
        'schema':'AURA-EVIDENCE-SLICE-DAG-CAMPAIGN-v1',
