...
def run(n=1000):
    false_reuse=0; counts={}
    for i in range(n):
        a=adapter(); a.store(fixture()); p=fixture(); mode=i%10
        if mode==0: p=replace(p,currentness_root=f"mut-cur:{i}")
        elif mode==1: p=replace(p,claim_digest=f"mut-cd:{i}")
        elif mode==2: p=replace(p,source_generation=f"mut-g:{i}")
        elif mode==3: p=replace(p,semantic_admission_root=f"mut-sa:{i}")
        elif mode==4: p=replace(p,evidence_roots=frozenset({"e1",f"mut-e:{i}"}))
        elif mode==5: p=replace(p,proof_surface={**p.proof_surface,"source.py":f"mut-b:{i}"})
        elif mode==6: p=replace(p,proof_surface={**p.proof_surface,f"new:{i}.py":"x"})
        elif mode==7: p=replace(p,resolution=1)
        elif mode==8: p=replace(p,currentness_root=f"mut-cur:{i}",proof_surface={**p.proof_surface,"README.md":f"mut-d:{i}"})
        elif mode==9: pass
        try: disposition=a.assess(p).disposition
        except ValueError: disposition="REJECT"
        counts[disposition]=counts.get(disposition,0)+1
        if (mode==9 and disposition!="REUSE_EXACT") or (mode!=9 and disposition=="REUSE_EXACT"):
            false_reuse+=1
    ...
    body={"n":n,"counts":counts,"false_reuse":false_reuse,"omega_keep":omega_keep,"tail_repairs":tail_repairs}
