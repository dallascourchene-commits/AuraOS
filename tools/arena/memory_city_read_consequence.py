...
class ReadWorldBinding:
    ...
    def binding_root(self):
        return digest({'schema':'AURA-MEMORY-CITY-READ-WORLD-BINDING-v1','hydration_receipt_root':self.hydration.receipt_root,'hydration_support_root':self.hydration.support_root,'typed_closure_receipt_root':self.typed_closure.receipt_root,'typed_reproof_semantics':self.typed_closure.reproof_semantics})
@dataclass(frozen=True)
...
def compile_read_consequence_certificate(worlds:tuple[ReadWorldBinding,...],coverage:CoverageCertificate)->ReadConsequenceCertificate:
    ...
    for w in worlds:
        ...
        if w.typed_closure.reproof_semantics!=REPROOF_SEMANTICS:return _hold('typed_reproof_semantics_not_current',coverage,bindings)
...
def validate_read_consequence_at_use(cert:ReadConsequenceCertificate,*,hydration:SupportClosedHydration,typed_closure:TypedClosureCertificate,fixed_support_root:str,coverage:CoverageCertificate,current_program_root:str,current_sealed_domain_root:str,current_coverage_generation:int,mutation_requested:bool=False)->ReadConsequenceUseDecision:
    ...
    if not isinstance(typed_closure,TypedClosureCertificate):raise ValueError('typed_closure must be TypedClosureCertificate')
    if typed_closure.reproof_semantics!=REPROOF_SEMANTICS:return ReadConsequenceUseDecision('HOLD_REPROOF_SEMANTICS','active_typed_reproof_semantics_not_current',cert.receipt_root)
    ...
    if typed_closure.disposition is not TypedClosureDisposition.READY:return ReadConsequenceUseDecision('HOLD_TYPED_CLOSURE','active_typed_closure_not_ready',cert.receipt_root)
    active_binding=ReadWorldBinding(hydration,typed_closure)
    if active_binding.binding_root not in cert.binding_roots:return ReadConsequenceUseDecision('HOLD_ACTIVE_WORLD_MEMBERSHIP','active_read_world_binding_not_certified',cert.receipt_root)
    ...
    if tuple(hydration.support_cut)!=cert.hydration_cut or tuple(typed_closure.reproof_item_ids)!=cert.reproof_item_ids:return ReadConsequenceUseDecision('HOLD_ACTIVE_CONSEQUENCE','active_hydration_or_reproof_consequence_moved',cert.receipt_root)
    if (typed_closure.transition_model_root,typed_closure.horizon,typed_closure.future_congruence_root)!=(cert.transition_model_root,cert.horizon,cert.future_congruence_root):return ReadConsequenceUseDecision('HOLD_TRANSITION_CONSEQUENCE','active_transition_or_horizon_consequence_moved',cert.receipt_root)
