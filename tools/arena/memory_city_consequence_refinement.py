...
def _root(v,n):
    if not isinstance(v,str) or len(v)!=64 or v.lower()!=v or any(c not in '0123456789abcdef' for c in v): raise ValueError(f'{n} must be lowercase sha256')
    return v
def _nn(v,n):
...
class EffectObligationProjection:
    ...
    def __post_init__(self):
        ...
        if type(self.obligation_authenticated) is not bool or type(self.observation_current) is not bool: raise ValueError('currentness flags must be bool')
...
class EffectRefinementPlan:
    ...
    gate10:bool=False
    def __post_init__(self):
        _root(self.read_certificate_root,'read_certificate_root');_root(self.parent_read_root,'parent_read_root');_root(self.intent_binding_root,'intent_binding_root');_root(self.plan_root,'plan_root')
...
def parent_read_root(cert)->str:
    ...
    return digest({'schema':SCHEMA,'kind':'parent_read','receipt_root':receipt,'coverage_receipt_root':coverage,'program_root':program,'sealed_domain_root':domain,'coverage_generation':generation,'binding_roots':bindings,'consequence_root':consequence,'transition_model_root':transition,'horizon':horizon,'future_congruence_root':future})

def _hold(cert,intent,status,reason,unresolved):
    rr=_root(getattr(cert,'receipt_root',None),'read_receipt_root'); parent=parent_read_root(cert); ib=intent.binding_root; unresolved=tuple(sorted(set(unresolved)))
    pr=digest({'schema':SCHEMA,'kind':'effect_refinement_hold','status':status,'reason':reason,'read_certificate_root':rr,'parent_read_root':parent,'intent_binding_root':ib,'unresolved':unresolved})
...
def compile_effect_refinement(projections:tuple[EffectObligationProjection,...], cert, intent:EffectIntent)->EffectRefinementPlan:
    ...
    for obligation,ps in sorted(groups.items()):
        ...
        sr=digest({'schema':SCHEMA,'kind':'effect_subclass','parent_read_root':parent,'read_certificate_root':rr,'intent_binding_root':ib,'obligation_root':obligation,'member_binding_roots':mb,'evidence_roots':ev})
    ...
    pr=digest({'schema':SCHEMA,'kind':'effect_refinement','status':status,'read_certificate_root':rr,'parent_read_root':parent,'intent_binding_root':ib,'children':[x.subclass_root for x in subclasses],'unresolved':unresolved})
...
def validate_effect_binding(plan:EffectRefinementPlan, *, cert, intent:EffectIntent, read_binding_root:str, obligation_root:str)->str:
    ...
    if parent_read_root(cert)!=plan.parent_read_root or getattr(cert,'receipt_root',None)!=plan.read_certificate_root:return 'HOLD_READ_CERTIFICATE_MOVED'
...
def effect_escalation_root(plan:EffectRefinementPlan, child:EffectSubclass)->str:
    ...
    return digest({'schema':SCHEMA,'kind':'effect_escalation','read_certificate_root':plan.read_certificate_root,'parent_read_root':plan.parent_read_root,'intent_binding_root':plan.intent_binding_root,'plan_root':plan.plan_root,'subclass_root':child.subclass_root,'obligation_root':child.obligation_root,'authority':D0})
...
def hard13d_refinement(axes):
    if len(axes)!=13 or any(type(x)is not int or x not in (0,1,2) for x in axes):return 'HOLD_MALFORMED'
    if 0 in axes[:8]:return 'HOLD_HARD_INVALID'
    if 1 in axes[:8]:return 'HOLD_UNRESOLVED'
    return 'READY_D0'
