...
def compile_hard_support_quotient(
    ...
    *,
    ...
    return HardSupportQuotient("READY_HARD_SUPPORT_QUOTIENT_D0", support_generation, components, lifted, item_map, digest(payload))


def _hold(status, generation, items, *, item_to_cid=None):
    item_map = tuple(sorted((item_to_cid or {x: x for x in items}).items()))
    payload={"disposition":status,"support_generation":generation,"items":items,"item_to_component":item_map}
    return HardSupportQuotient(status,generation,(),(),item_map,digest(payload))
...
def validate_contingent_strategy_branch(
    ...
    *,
    ...
    """Bind the parent contingent-hydration branch to the exact support quotient.

    Duck-typed intentionally to avoid taking ownership of the parent compiler. The
    parent remains the nonanticipatory capacity/deadline policy owner; this gate
    verifies that its branch support partition matches this quotient and that the
    required quotient components are current/obtainable by the decision boundary.
    """
    if q.disposition != "READY_HARD_SUPPORT_QUOTIENT_D0":
        return _decision("HOLD_QUOTIENT_NOT_READY",(),(),(),(),(),q.quotient_root)
    ...
    expected = {
        ...
        if c.component_id in set(support_components_for_items(q, required_items))
    }
    actual = {tuple(sorted(c)) for c in getattr(plan, "required_components", ())}
    if expected != actual:
        return _decision("HOLD_SUPPORT_PARTITION_MISMATCH",tuple(sorted(support_components_for_items(q, required_items))),(),(),(),(),q.quotient_root)
