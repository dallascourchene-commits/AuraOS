...
"""D0 bridge from Memory City typed closure/reuse proof to mutation authority.
...
It mints no effect, truth, lease, or Gate-10 authority.
"""
...
SCHEMA = "AURA-MEMORY-CITY-HORIZON-FENCED-HANDOFF-v1"
D0 = "D0_NONPROMOTING"
...
def _disposition_value(obj):
    raw = getattr(obj, "disposition", None)
    return getattr(raw, "value", raw)


def semantic_handoff_root(cert) -> str:
    """Bind the exact semantic proof whose consequence reaches the mutation seam."""
    ...
    receipt_root = _root(getattr(cert, "receipt_root", None), "closure_receipt_root")
    horizon = getattr(cert, "horizon", None)
    if type(horizon) is not int or horizon < 0: raise ValueError("horizon must be nonnegative exact int")
    ...
    return digest({
        ...
        "closure_receipt_root": receipt_root,
    })
...
def read_use_root(cert, use_decision) -> str:
    return digest({
        ...
        "disposition": _disposition_value(use_decision),
...
def compile_horizon_fenced_handoff(cert, use_decision, evidence: SemanticHandoffEvidence,
    ...
    if _disposition_value(cert) != "READY_D0" or _disposition_value(use_decision) != "READY_D0":
