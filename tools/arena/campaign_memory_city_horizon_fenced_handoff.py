...
def main():
    keys=['cases','oracle_ready','bridge_ready','legacy_typed_only_incompatible','uncrossbound_false_ready','bridge_false_ready','bridge_false_hold','full_config_false_hold','consequence_swap','coverage_move','member_move','horizon_move','fence_attack','receipt_attack','valid_irrelevant_context']
    m={k:0 for k in keys}
    for n in range(12000):
        ...
        if cls==0:
            ...
            old=ReadCert(**{**c.__dict__,'consequence_root':R(['old-c',tag]),'receipt_root':R(['old-r',tag])}); mut=MutationBoundaryProjection(mut.cell_id,mut.revision,mut.configuration_root,mut.support_epoch,mut.fence_generation,mut.installed_fence_generation,mut.holder,mut.expires_at,semantic_handoff_root(old),mut.transition_authority_receipt_root,mut.resource_fence_receipt_root); m['consequence_swap']+=1
        elif cls==2:
            old=ReadCert(**{**c.__dict__,'coverage_generation':3,'receipt_root':R(['old-r2',tag])}); mut=MutationBoundaryProjection(mut.cell_id,mut.revision,mut.configuration_root,mut.support_epoch,mut.fence_generation,mut.installed_fence_generation,mut.holder,mut.expires_at,semantic_handoff_root(old),mut.transition_authority_receipt_root,mut.resource_fence_receipt_root); m['coverage_move']+=1
        elif cls==3:
            old=ReadCert(**{**c.__dict__,'member_support_roots':(R(['s1',tag]),R(['s3',tag])),'receipt_root':R(['old-r3',tag])}); mut=MutationBoundaryProjection(mut.cell_id,mut.revision,mut.configuration_root,mut.support_epoch,mut.fence_generation,mut.installed_fence_generation,mut.holder,mut.expires_at,semantic_handoff_root(old),mut.transition_authority_receipt_root,mut.resource_fence_receipt_root); m['member_move']+=1
        elif cls==4:
    ...
    out={'schema':'AURA-MEMORY-CITY-HFSC-R3-CAMPAIGN-v1','metrics':m};out['campaign_root']=hashlib.sha256(json.dumps(out,sort_keys=True,separators=(',',':')).encode()).hexdigest();print(json.dumps(out,sort_keys=True,separators=(',',':')))
