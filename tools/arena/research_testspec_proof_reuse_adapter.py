...
class ResearchProofContract:
    ...
    def validate(self, *, stored: bool) -> None:
        ...
        if self.min_independent_roots < 1 or len(self.evidence_roots) < self.min_independent_roots:
...
class ResearchTestSpecProofReuseAdapter:
    ...
    def store(self, c: ResearchProofContract) -> StoredResearchProof:
        ...
        semantic_root=_digest({
            ...
            "source":[c.source_id,c.source_owner_ref,c.source_generation],"currentness":c.currentness_root,
            "evidence":sorted(c.evidence_roots),"resolution":c.resolution,
            "surface":dict(sorted(c.proof_surface.items())),
    ...
    def assess(self, probe: ResearchProofContract) -> ReuseDecision:
        ...
        hard_identity=any(r in reasons for r in (
            ...
            "SOURCE_GENERATION_DRIFT","EVIDENCE_ANCESTRY_DRIFT","RESOLUTION_TOO_SHALLOW",
        ))
        ...
        source=SourceExit(probe.source_id,probe.source_owner_ref,probe.source_generation,
                          probe.semantic_admission_root,current=True)
        policy=AdmissionPolicy("RESEARCH_TESTSPEC_REUSE",tuple(range(8)),tuple(sorted(set().union(*(u.dependencies for u in base.proof_units)))))
