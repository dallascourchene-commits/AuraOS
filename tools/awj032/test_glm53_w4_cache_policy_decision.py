...
class W4CachePolicyDecisionTests(unittest.TestCase):
    def preflight(
        ...
        *,
        ...
        overlap=0.0,
        queue=0.05,
        source="glm53-source-generation",
        workload="trace-A",
        scope="same-host-fixture",
        attested=True,
    ):
        return evaluate_w4_counters(
            W4CounterSnapshot(
                scope_ref=scope,
    ...
    def test_candidate_must_pareto_dominate_before_replacing_baseline(self):
        ...
        candidate = self.obs(
            "ADAPTIVE_RECENCY_FREQUENCY",
            policy_class="ADAPTIVE",
            preflight=self.preflight(demand=600, aura_avoided=400),
            hit=0.5,
            energy=8.0,
            memory=900,
            warmup=1.5,
    ...
    def test_k27_higher_hit_rate_does_not_override_lifecycle_tradeoff(self):
        ...
        k27 = self.obs(
            "K27_GEOMETRIC",
            policy_class="K27_GEOMETRIC",
            preflight=self.preflight(demand=700, aura_avoided=300),
            hit=0.90,
            energy=14.0,
            memory=1800,
            warmup=4.0,
    ...
    def test_higher_hit_ratio_cannot_save_pareto_regression(self):
        ...
        candidate = self.obs(
            "TEMPORAL_PREDICTIVE_CACHE",
            policy_class="PREDICTIVE_CACHE",
            hit=0.95,
            energy=12.0,
            memory=1400,
            warmup=3.0,
    ...
    def test_source_or_workload_substitution_fails(self):
        ...
        self.assert_code(
            "SAME_SCOPE_SOURCE_WORKLOAD_REQUIRED",
    ...
    def test_prefetch_bytes_cannot_receive_cache_policy_credit(self):
        ...
        self.assert_code(
            "CANDIDATE_PREFETCH_CROSS_CREDIT_FORBIDDEN",
            lambda: compare_cache_policy_to_baseline(baseline=baseline, candidate=candidate),
        )
    ...
    def test_latency_overlap_cannot_receive_cache_policy_credit(self):
        ...
        self.assert_code(
            "CANDIDATE_OVERLAP_CROSS_CREDIT_FORBIDDEN",
            lambda: compare_cache_policy_to_baseline(baseline=baseline, candidate=candidate),
        )
    ...
    def test_stale_measurement_fails_closed(self):
        ...
        self.assert_code(
            "CANDIDATE_MEASUREMENT_CURRENT_REQUIRED",
            lambda: compare_cache_policy_to_baseline(baseline=baseline, candidate=candidate),
        )
    ...
    def test_truthy_integer_cannot_impersonate_currentness_boolean(self):
        ...
        self.assert_code(
            "CANDIDATE_MEASUREMENT_CURRENT_INVALID",
            lambda: compare_cache_policy_to_baseline(baseline=baseline, candidate=candidate),
        )
    ...
    def test_unattested_w4_receipt_fails(self):
        ...
        self.assert_code(
            "CANDIDATE_PHYSICAL_IO_ATTESTATION_REQUIRED",
            lambda: compare_cache_policy_to_baseline(baseline=baseline, candidate=candidate),
        )
