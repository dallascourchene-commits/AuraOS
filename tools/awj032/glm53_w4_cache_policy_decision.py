...
Consumes already-attested W4 expert-I/O counter receipts and compares one cache
policy against an explicit baseline on the exact same source/workload scope.
The comparator is deliberately weight-free: a candidate may replace the
baseline only by Pareto dominance across measured lifecycle costs.
...
class W4CachePolicyDecisionReceipt:
    ...
    claim_ceiling: str = (
        "D0_SAME_SCOPE_CACHE_POLICY_PARETO_DECISION_ONLY_"
...
def _validate_observation(obs: W4CachePolicyObservation, *, label: str) -> dict[str, float]:
    ...
    # This decision lane is cache-placement only. Prefetch and latency overlap are
    # separate responsibility planes and cannot earn cache-policy credit here.
    ...
    return {
        "physical_total_expert_bytes": float(_bytes(
            f"{label}_physical_total_expert_bytes",
            obs.preflight.physical_total_expert_bytes,
        )),
...
def compare_cache_policy_to_baseline(
    *,
    baseline: W4CachePolicyObservation,
    candidate: W4CachePolicyObservation,
) -> W4CachePolicyDecisionReceipt:
    """Compare exact same-scope cache policies without invented scalar weights."""
    ...
    bp = baseline.preflight
    cp = candidate.preflight
    same_scope = (
    ...
    if better and not worse:
        relation = "CANDIDATE_PARETO_DOMINATES_BASELINE"
        retained = candidate.policy_id
    elif worse and not better:
        relation = "CANDIDATE_PARETO_REGRESSES_BASELINE"
        retained = baseline.policy_id
    elif not better and not worse:
        relation = "CANDIDATE_EQUAL_TO_BASELINE"
        retained = baseline.policy_id
    else:
        relation = "NONDOMINATED_TRADEOFF_REQUIRES_EXPLICIT_POLICY_PREFERENCE"
        retained = None
