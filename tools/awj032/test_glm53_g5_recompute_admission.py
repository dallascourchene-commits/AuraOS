...
class GLM53G5RecomputeAdmissionTests(unittest.TestCase):
    def g4(self, *, source_changed: bool = False, unchanged: bool = False) -> G4RevalidationProjection:
        if unchanged:
            return G4RevalidationProjection(
                receipt_digest=d("a"),
                disposition=G4_REVALIDATED_UNCHANGED,
                changed_axes=(),
    ...
    def progress(self, disposition: str = PROGRESS_ALLOW_STATE_TRANSITION) -> RetrievalProgressProjection:
    ...
    def currentness(self, state: str = READ_RESOLVED_CURRENT, *, source: str = "source::18") -> SourceReadCurrentnessProjection:
    ...
    def test_unchanged_g4_plan_makes_recompute_not_applicable(self) -> None:
        receipt = self.assess(
            g4=self.g4(unchanged=True),
            progress=self.progress(PROGRESS_ALLOW_STATE_TRANSITION),
            currentness=self.currentness(source="source::17"),
        )
        self.assertEqual(receipt.disposition, NOT_APPLICABLE_RECOMPUTE)
        self.assertFalse(receipt.bounded_g3_recompute_attempt_admitted)

    def test_no_progress_first_repeat_holds_before_recompute(self) -> None:
    ...
    def test_non_source_runtime_drift_with_independent_progress_admits_bounded_attempt(self) -> None:
        for disposition in (PROGRESS_ALLOW_INITIAL, PROGRESS_ALLOW_CHANGED_AXIS, PROGRESS_ALLOW_STATE_TRANSITION):
            with self.subTest(disposition=disposition):
                receipt = self.assess(progress=self.progress(disposition))
                self.assertEqual(receipt.disposition, ADMIT_BOUNDED_G3_RECOMPUTE_ATTEMPT)
                self.assertTrue(receipt.bounded_g3_recompute_attempt_admitted)
    ...
    def test_source_version_transition_without_read_currentness_holds(self) -> None:
        receipt = self.assess(g4=self.g4(source_changed=True), version=self.version())
    ...
    def test_source_drift_with_version_edge_and_current_read_witness_admits(self) -> None:
        receipt = self.assess(
            g4=self.g4(source_changed=True),
            version=self.version(),
    ...
    def test_no_progress_dominates_version_currentness_surplus(self) -> None:
        receipt = self.assess(
            g4=self.g4(source_changed=True),
    ...
    def test_version_transition_must_carry_future_read_debt(self) -> None:
        with self.assertRaises(ValueError):
            replace(self.version(), future_read_currentness_required=False).validate()

    def test_version_projection_cannot_self_authenticate_source(self) -> None:
        with self.assertRaises(ValueError):
            replace(self.version(), source_owner_authenticated_by_this_contract=True).validate()
        with self.assertRaises(ValueError):
            replace(self.version(), source_currentness_proven_by_this_contract=True).validate()

    def test_currentness_projection_cannot_self_authenticate_owner(self) -> None:
        with self.assertRaises(ValueError):
            replace(self.currentness(), authenticated_by_this_contract=True).validate()
    ...
    def test_finite_recompute_lattice_exhausts_90_valid_states(self) -> None:
        proof = prove_finite_recompute_lattice()
        self.assertEqual(proof["states"], 90)
        self.assertGreater(proof[ADMIT_BOUNDED_G3_RECOMPUTE_ATTEMPT], 0)
