...
def build_canonical_role_intent(sources: GLM53CheckpointSourceBundle) -> tuple[CheckpointExtraLayerClassification, CheckpointExtraLayerEvidenceObservation]:
...
def build_final_source_bound_report(sources: GLM53CheckpointSourceBundle, *, observation_time: str | None = None) -> dict[str, Any]:
    ...
    if report.get("blockers") != [RESOLVER_PROVENANCE_BLOCKER]:
        raise PR340ProducerSnapshotError("FINAL_REPORT_BLOCKER_SET_MISMATCH", repr(report.get("blockers")))
    ...
    if report.get("classified_extra_checkpoint_layers") != [{"index": ROLE_INDEX, "role": ROLE, "decoder_pager_membership": False}]:
...
def emit_pr340_producer_snapshot(sources: GLM53CheckpointSourceBundle, *, producer_execution_head: str, observation_time: str | None = None) -> tuple[PR340ProducerSnapshot, dict[str, Any]]:
