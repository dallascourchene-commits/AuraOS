...
SENSITIVE_ENV_FRAGMENT = re.compile(
    r"TOKEN|SECRET|PASSWORD|CREDENTIAL|PRIVATE|API_KEY", re.IGNORECASE
)
AUTHORITY_CONTRACT = {
...
def _external_output_path(root: Path, value: str | Path) -> Path:
    ...
    try:
        ...
        return output
    raise RuntimeHarnessError(
        "runtime evidence output must be outside the repository checkout"
    )
...
def _validate_loopback_url(value: Any, label: str) -> str:
    ...
    if parsed.username or parsed.password or not parsed.port:
        raise RuntimeHarnessError(
            f"{label} must include a port and no credentials"
        )
    return value
...
def _validate_command(value: Any, label: str) -> tuple[str, ...]:
    if not isinstance(value, list) or not value or len(value) > MAX_COMMAND_ARGS:
        raise RuntimeHarnessError(
            f"{label} must be a non-empty bounded argument array"
        )
    output: list[str] = []
    for item in value:
...
def _validate_env(value: Any, label: str) -> dict[str, str]:
    ...
    for key, item in value.items():
        if not isinstance(key, str) or not re.fullmatch(
            ...
            raise RuntimeHarnessError(
                f"{label} contains an invalid environment key"
            )
        if SENSITIVE_ENV_FRAGMENT.search(key):
            raise RuntimeHarnessError(
                f"{label} may not declare secret-bearing environment keys"
            )
        if (
...
def load_runtime_profile(
    root: Path, profile_path: str | Path
) -> dict[str, Any]:
    root = root.resolve()
    path = _safe_repo_path(root, str(profile_path), "runtime profile")
    if not path.is_file() or path.is_symlink():
        raise RuntimeHarnessError(
            f"runtime profile is missing or unsafe: {path}"
        )
    ...
    try:
        ...
        raise RuntimeHarnessError(
            f"runtime profile is not canonical UTF-8 JSON: {exc}"
        ) from exc
    if not isinstance(raw, dict) or raw.get("version") != PROFILE_VERSION:
        raise RuntimeHarnessError(
            f"runtime profile version must be {PROFILE_VERSION}"
        )
    profile_id = raw.get("profile_id")
    if not isinstance(profile_id, str) or not re.fullmatch(
        r"[a-z0-9][a-z0-9._-]{0,63}", profile_id
    ):
    ...
    if not isinstance(requirements, list) or len(requirements) > 8:
        raise RuntimeHarnessError(
            "runtime requirements must be a bounded array"
        )
    requirement_paths: list[str] = []
    for index, requirement in enumerate(requirements):
        target = _safe_repo_path(
            root, requirement, f"requirements[{index}]"
        )
        if not target.is_file() or target.is_symlink():
            raise RuntimeHarnessError(
                f"requirements file is missing or unsafe: {requirement}"
            )
    ...
    if not isinstance(server, dict) or not isinstance(probe, dict):
        raise RuntimeHarnessError(
            "runtime profile requires server and probe objects"
        )
    server_command = _validate_command(
        server.get("command"), "server.command"
    )
    readiness_url = _validate_loopback_url(
        server.get("readiness_url"), "server.readiness_url"
    )
    ...
    probe_command = _validate_command(
        probe.get("command"), "probe.command"
    )
    ...
    if (
        ...
        or len(required_artifacts) > MAX_REQUIRED_ARTIFACTS
    ):
        raise RuntimeHarnessError(
            "probe.required_artifacts must be a bounded array"
        )
    ...
    for index, item in enumerate(required_artifacts):
        if (
            ...
            raise RuntimeHarnessError(
                f"probe.required_artifacts[{index}] is invalid"
            )
        pure = PurePosixPath(item)
        if pure.is_absolute() or ".." in pure.parts:
            raise RuntimeHarnessError(
                "probe artifact path escapes the output directory"
            )
    ...
    if success_json is not None and success_json not in canonical_artifacts:
        raise RuntimeHarnessError(
            "probe.success_json must be one of the required artifacts"
        )
    success_field = probe.get("success_field", "ok")
    if not isinstance(success_field, str) or not re.fullmatch(
        r"[A-Za-z_][A-Za-z0-9_.]{0,127}", success_field
    ):
        raise RuntimeHarnessError("probe.success_field is invalid")

    verification_raw = raw.get("verification_commands") or []
    if (
        not isinstance(verification_raw, list)
        or len(verification_raw) > MAX_COMMANDS
    ):
        raise RuntimeHarnessError(
            "verification_commands must be a bounded array"
        )
    verification: list[dict[str, Any]] = []
    for index, row in enumerate(verification_raw):
        if isinstance(row, list):
            command = _validate_command(
                row, f"verification_commands[{index}]"
            )
            ...
            raise RuntimeHarnessError(
                f"verification_commands[{index}] is invalid"
            )
        verification.append(
    ...
    return {
        ...
        "axiom_bindings": tuple(
            raw.get("axiom_bindings") or AXIOM_BINDINGS
        ),
    }
...
def _safe_environment(
    root: Path, additions: Mapping[str, str] | None = None
) -> dict[str, str]:
    env = {
        key: value
...
def _substitute(
    value: str, *, root: Path, output: Path, python: Path
) -> str:
    return (
        value.replace("{repo}", str(root))
...
def _resolve_command(
    command: Sequence[str], *, root: Path, output: Path, python: Path
) -> tuple[str, ...]:
    values = tuple(
        _substitute(item, root=root, output=output, python=python)
        for item in command
    )
    executable = values[0]
    if executable == str(python):
        ...
        if not candidate.is_absolute():
            try:
                ...
                raise RuntimeHarnessError(
                    "command executable escapes the repository"
                ) from exc
    else:
    ...
    if not resolved.is_file():
        raise RuntimeHarnessError(
            f"runtime executable is not a file: {resolved}"
        )
...
def _artifact_inventory(
    output: Path, paths: Sequence[str]
) -> list[dict[str, Any]]:
    ...
    for item in paths:
        ...
        try:
            ...
            raise RuntimeHarnessError(
                "runtime artifact escaped the output directory"
            ) from exc
        row: dict[str, Any] = {
            "path": item,
...
def _load_baseline(
    ...
    if (
        ...
        or target.stat().st_size > MAX_RECEIPT_BYTES
    ):
    ...
    if (
        ...
        or not value.get("run_digest")
    ):
        raise RuntimeHarnessError(
            "baseline receipt is not an Aura runtime harness receipt"
        )
    return {
        "path": str(target),
...
def run_runtime_profile(
    ...
    *,
    ...
    if (
        ...
        and before.get("status")
        and not allow_dirty
    ):
        raise RuntimeHarnessError(
            "repository is dirty; runtime evidence must bind a clean tree"
        )
    ...
    try:
        ...
        raise RuntimeHarnessError(
            "runtime venv must be outside the repository checkout"
        )
    ...
    if install_requirements:
        for index, requirement in enumerate(
            ...
            if result["returncode"] != 0:
                raise RuntimeHarnessError(
                    f"requirements installation failed: {requirement}"
                )
    ...
    try:
        ...
        if readiness["ok"]:
            probe_env_values = {
                key: _substitute(
                    value, root=root, output=output, python=python
                )
    ...
    artifacts = _artifact_inventory(
        output, profile["probe"]["required_artifacts"]
    )
    success_ok = True
    success_value: Any = None
    if profile["probe"]["success_json"]:
        success_path = output / Path(
            *PurePosixPath(profile["probe"]["success_json"]).parts
        )
        success_ok, success_value = _read_success_field(
            success_path, profile["probe"]["success_field"]
        )
    ...
    if baseline and baseline.get("profile_id") != profile["profile_id"]:
        raise RuntimeHarnessError(
            "baseline receipt profile does not match current runtime profile"
        )
    cycle_state = (
