...
def registry_oracle(dbpath: Path, manifest_path: Path) -> dict:
    ...
    with MemoryStore(dbpath, read_only=True) as db:
        ...
        if len(all_rows)!=1115 or len(routes)!=1000: raise SystemExit('registry shape mismatch')
        for i in range(10):
            prefix=(2,i)
    ...
    if hashlib.sha256(manifest_bytes).hexdigest()!=EXPECTED_COLD_SOURCE_MANIFEST_SHA256: raise SystemExit('cold source manifest identity mismatch')
    ...
    with tempfile.TemporaryDirectory(prefix='k27-oracle-copy-') as td:
        ...
        with MemoryStore(copied) as db:
            ...
            result=db.publish(changed,{**record['payload'],'synthetic_validation_change':True},FrameAddress(a['frame_id'],a['frame_generation'],tuple(a['path']),changed),source_url=record['source_url'],source_version=record['source_version']+'/synthetic-test',expected_revision=record['revision_id'],expected_epoch=record['epoch'])
            ...
            for key,prior in records.items():
                ...
                elif key!=changed and (now['state']!='fresh' or now['revision_id']!=prior['revision_id']): raise SystemExit('unaffected record drift')
...
def main():
    ...
    frozen_source=payload/'frozen_68'; frozen_manifest_path=payload/'frozen_68_manifest.json'
    frozen_manifest=json.loads(frozen_manifest_path.read_text(encoding='utf-8'))
    if frozen_manifest.get('schema')!='aura-k27-frozen-68-manifest-v1' or frozen_manifest.get('file_count')!=18: raise SystemExit('frozen 68 manifest mismatch')
    with tempfile.TemporaryDirectory(prefix='k27-candidate-verify-') as td:
        td=Path(td); frozen=td/'frozen_68'; shutil.copytree(frozen_source,frozen)
        missing=frozen/'reader/frozen-store.json'
        if not missing.exists():
            parts=payload/'frozen_68_parts/reader'; part_paths=sorted(parts.glob('frozen-store.json.part*'))
            if len(part_paths)!=3: raise SystemExit('frozen store transport parts missing')
            missing.parent.mkdir(parents=True,exist_ok=True); missing.write_bytes(b''.join(part.read_bytes() for part in part_paths))
        for entry in frozen_manifest['files']:
            target=frozen/entry['path']
            if not target.is_file() or target.is_symlink() or target.stat().st_size!=entry['size'] or hashlib.sha256(target.read_bytes()).hexdigest()!=entry['sha256']: raise SystemExit(f"frozen 68 file identity mismatch: {entry['path']}")
        cache=td/'runtime-cache'; import os
        old_cache=os.environ.get('AURA_K27_RUNTIME_CACHE'); os.environ['AURA_K27_RUNTIME_CACHE']=str(cache)
        try:
            runtime=K27MemoryCityRuntime(root); dbpath=runtime.registry_path
        finally:
        ...
        receipt={'schema':'aura-k27-memory-city-candidate-verification-v2','baseline_tests_passed':68,'baseline_tests_failed':0,'registry_checks_passed':oracle['checks_passed'],'cold_source_hash_bindings_present':oracle['cold_source_hash_bindings_present'],'runtime_binding_tests_passed':6,'runtime_binding_tests_failed':0,'source_database_sha256':EXPECTED_SOURCE_DATABASE_SHA256,'cold_source_manifest_sha256':EXPECTED_COLD_SOURCE_MANIFEST_SHA256,'registry_recipe_version':EXPECTED_RECIPE_VERSION,'runtime_database_sha256':hashlib.sha256(dbpath.read_bytes()).hexdigest(),'semantic_registry_root':EXPECTED_SEMANTIC_ROOT,'authority_minted':False,'gate10':False,'canonical_promotion':False,'cas_smoke_rounds':3,'commands':[name for name,_,__ in out],'frozen_68_files_verified':18,'cold_source_byte_verification':'Lane-1 provenance evidence; not self-reverified by Lane-2 hosted candidate'}
