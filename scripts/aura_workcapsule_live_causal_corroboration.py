...
def _authority_violations(
    receipt: dict[str, Any], expected_fields: set[str], prefix: str
) -> list[str]:
...
def _flag_violations(
    receipt: dict[str, Any], true_fields: set[str], false_fields: set[str], prefix: str
) -> list[str]:
    violations: list[str] = []
...
def _verify_pr572(receipt: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(identity, dict):
        ...
        if set(identity) != expected_identity_fields:
            ...
            or identity.get("scope_profile") != PR572_VERSION
...
def verify_live_causal_corroboration(
    *, pr568_receipt: dict[str, Any], pr572_receipt: dict[str, Any]
) -> list[str]:
    ...
    source568 = (
        pr568_receipt["dependency_key"]["file_id"],
        pr568_receipt["dependency_key"]["relative_path"],
        pr568_receipt["source_generation"],
        pr568_receipt["full_source_sha256_hex"],
        pr568_receipt["full_source_byte_len"],
    )
    source572 = (
        pr572_receipt["file_id"],
        pr572_receipt["relative_path"],
        pr572_receipt["source_generation"],
        pr572_receipt["full_source_sha256_hex"],
        pr572_receipt["full_source_byte_len"],
    )
    ...
    target572 = (
        pr572_receipt["target_byte_start"],
        pr572_receipt["target_byte_end"],
        pr572_receipt["target_slice_sha256_hex"],
        pr572_receipt["selected_target_semantic_handle_digest_hex"],
    )
    if target568 != target572:
        violations.append("LIVE_TARGET_SLICE_MISMATCH")

    if pr568_receipt["post_source_projection_receipt_identity"] != pr572_receipt[
        "post_source_projection_receipt_identity"
    ]:
        violations.append("POST_SOURCE_PROJECTION_IDENTITY_MISMATCH")
    if pr568_receipt["causal_post_closure_receipt_identity"] != pr572_receipt[
        "causal_post_o10_receipt_identity"
    ]:
...
def admit_live_causal_corroboration(
    *, pr568_receipt: dict[str, Any], pr572_receipt: dict[str, Any]
) -> dict[str, Any]:
    violations = verify_live_causal_corroboration(
        pr568_receipt=pr568_receipt, pr572_receipt=pr572_receipt
    )
    if violations:
    ...
    out = {
        ...
        "source_instance": {
            "file_id": pr572_receipt["file_id"],
            "relative_path": pr572_receipt["relative_path"],
            "source_generation": pr572_receipt["source_generation"],
            "full_source_sha256_hex": pr572_receipt["full_source_sha256_hex"],
            "full_source_byte_len": pr572_receipt["full_source_byte_len"],
        },
        "target_slice": {
            "target_byte_start": pr572_receipt["target_byte_start"],
            "target_byte_end": pr572_receipt["target_byte_end"],
            "target_slice_sha256_hex": pr572_receipt["target_slice_sha256_hex"],
            "selected_target_semantic_handle_digest_hex": pr572_receipt[
                "selected_target_semantic_handle_digest_hex"
            ],
        },
