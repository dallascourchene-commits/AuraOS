...
class WorkCapsuleCausalArtifactQualifiedHostEnvelopeTests(WorkCapsuleArtifactQualifiedHostObservationTests):
    def causal_host_receipt(self, *, states=None, target_ref=None, **overrides) -> dict:
        receipt = copy.deepcopy(self.host_receipt(states=states, target_ref=target_ref))
        receipt.pop("receipt_identity", None)
    ...
    def causal_kwargs(self, *, host=None, raw=None) -> dict:
        ...
        return {**parent, "causal_host_admission_receipt": host if host is not None else self.causal_host_receipt()}

    def test_exact_causal_host_envelope_binds_to_exact_artifact(self) -> None:
        self.assertEqual([], verify_causal_artifact_qualified_host_envelope(**self.causal_kwargs()))
    ...
    def test_foreign_artifact_target_ref_rejects_even_when_causal_envelope_resealed(self) -> None:
        host = self.causal_host_receipt(target_ref="aura-workcapsule-target-sha256:" + "cd" * 32)
        violations = verify_causal_artifact_qualified_host_envelope(**self.causal_kwargs(host=host))
        self.assertTrue(any("RESOLVED_HOST_GATE_TARGET_REF_MISMATCH:U_HEAD" in item for item in violations))

    def test_causal_scar_cannot_be_resealed_away(self) -> None:
        for field, value in (("pre_reentry_receipt_reused_for_post_o10", False), ("fresh_post_reentry_receipt_substituted", True), ("causal_temporal_owner_reproved", False)):
    ...
    def test_bool_cannot_impersonate_fail_mask(self) -> None:
        ...
        _seal_causal(host)
        self.assertIn("FAIL_MASK_MISMATCH", verify_causal_host_admission_envelope(host))

    def test_nested_resolution_tamper_with_outer_reseal_still_rejects(self) -> None:
        host = self.causal_host_receipt()
        host["host_gate_resolutions"]["U_HEAD"]["target_ref"] = "aura-workcapsule-target-sha256:" + "ef" * 32
        _seal_causal(host)
        self.assertIn("HOST_RESOLUTION_DIGEST_MISMATCH:U_HEAD", verify_causal_host_admission_envelope(host))

    def test_all_pass_for_exact_artifact_stays_nonauthorizing(self) -> None:
        host = self.causal_host_receipt(states={gate: "PASS" for gate in self.host_receipt()["host_gate_states"]})
        out = admit_causal_artifact_qualified_host_envelope(**self.causal_kwargs(host=host))
    ...
    def test_host_effect_authority_widening_rejects_after_reseal(self) -> None:
        ...
        _seal_causal(host)
        self.assertTrue(any("host_effect_ready" in item for item in verify_causal_host_admission_envelope(host)))

    def test_public_boundary_replaces_old_host_envelope_name_only(self) -> None:
        params = set(inspect.signature(verify_causal_artifact_qualified_host_envelope).parameters)
        self.assertEqual({"scoped_target_inputs", "higher_owner_projection", "raw_slice_receipt", "causal_host_admission_receipt"}, params)
