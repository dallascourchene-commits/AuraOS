...
_NOW = datetime(2026, 8, 8, 0, 0, tzinfo=timezone.utc)
...
def _validate(value: dict) -> None:
    validate_arch_v2_3_capsule_semantics(value, now=_NOW)
...
def test_expired_enabled_jspace_is_rejected() -> None:
    value = _capsule()
    value["jspace_projection"]["expires_at"] = "2026-08-07T23:59:59Z"
    with pytest.raises(ValueError, match="JSpace projection is expired"):
        _validate(value)


def test_expired_validated_authorization_is_rejected() -> None:
    value = _capsule()
    value["commit_authorization"]["expires_at"] = "2026-08-07T23:59:59Z"
    with pytest.raises(ValueError, match="commit authorization is expired"):
        _validate(value)
