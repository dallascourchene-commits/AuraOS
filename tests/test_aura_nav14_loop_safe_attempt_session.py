...
class Nav14LoopSafeAttemptSessionTests(unittest.TestCase):
    ...
    def test_parent_generation_forgery_holds(self) -> None:
        ...
        for forged_h, forged_g in (
            ...
            r = m.bind_loop_safe_attempt_session(handoff=forged_h, ledger=l, guard=forged_g, intent=i)
    ...
    def test_candidate_identity_must_commute(self) -> None:
        ...
        for forged_l, forged_i in (
            ...
            r = m.bind_loop_safe_attempt_session(handoff=h, ledger=forged_l, guard=g, intent=forged_i)
            self.assertEqual(r.disposition, m.AttemptDisposition.HOLD_CANDIDATE_BINDING_MISMATCH)
    ...
    def test_session_identity_must_commute(self) -> None:
        ...
        for forged_l, forged_g, forged_i in (
            ...
            r = m.bind_loop_safe_attempt_session(handoff=h, ledger=forged_l, guard=forged_g, intent=forged_i)
            self.assertEqual(r.disposition, m.AttemptDisposition.HOLD_SESSION_BINDING_MISMATCH)
    ...
    def test_terminal_or_no_progress_ledger_debt_requires_reopen(self) -> None:
        ...
        for forged in (
            ...
            r = m.bind_loop_safe_attempt_session(handoff=h, ledger=forged, guard=g, intent=i)
            self.assertEqual(r.disposition, m.AttemptDisposition.HOLD_ATTEMPT_LEDGER_REOPEN_REQUIRED)
    ...
    def test_every_loop_guard_debt_taints_session(self) -> None:
        ...
        for forged in variants:
            r = m.bind_loop_safe_attempt_session(handoff=h, ledger=l, guard=forged, intent=i)
    ...
    def test_handoff_readiness_and_ceiling_fail_closed(self) -> None:
        ...
        r = m.bind_loop_safe_attempt_session(handoff=not_ready, ledger=l, guard=g, intent=i)
        ...
        for forged in (
            ...
            r = m.bind_loop_safe_attempt_session(handoff=forged, ledger=l, guard=g, intent=i)
        ...
        for forged_guard in (
            ...
            with self.assertRaises(ValueError):
                m.bind_loop_safe_attempt_session(handoff=h, ledger=l, guard=forged_guard, intent=i)

    def test_ledger_cannot_self_mint_authentication(self) -> None:
        h,l,g,i = m._fixture()
        with self.assertRaises(ValueError):
            m.bind_loop_safe_attempt_session(
                handoff=h, ledger=replace(l, producer_authenticated_by_this_contract=True), guard=g, intent=i
            )
