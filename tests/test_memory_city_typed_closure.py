...
class T(unittest.TestCase):
 ...
 def test_separate_hydration_and_reproof(self):
  i,b,s,g=self.base(); c=compile_typed_closure(i,b,s,g,changed_evidence_item_ids=('a',),max_resident_bytes=20,reveal_tick=0,deadline_tick=5,transition_model_root='t')
  self.assertEqual(c.hydration.branch_plans[0].required_item_ids,('a','b')); self.assertEqual(c.reproof_item_ids,('a','b','c','d'))
 def test_changed_support_peer_is_reproof_seed(self):
  i,b,s,g=self.base(); u=tuple(x.item_id for x in i); self.assertEqual(typed_reproof_descendants(('a',),g,s,u),('a','b','c','d')); self.assertEqual(directed_descendants(('a',),g,u),('a','c','d'))
 def test_destination_expands_whole_support_component(self):
 ...
 def test_direction_preserved_after_component_lift(self):
 ...
 def test_no_support_reduces_to_raw_reachability(self):
 ...
 def test_incomplete_influence_holds(self):
  i,b,s,g=self.base(); g=InfluenceGraph(1,g.edges,False); c=compile_typed_closure(i,b,s,g,changed_evidence_item_ids=('a',),max_resident_bytes=20,reveal_tick=0,deadline_tick=5,transition_model_root='t'); self.assertEqual(c.disposition,TypedClosureDisposition.HOLD_INCOMPLETE_RELATION)
 ...
 def test_influence_root_move_holds(self):
  i,b,s,g=self.base(); c=compile_typed_closure(i,b,s,g,changed_evidence_item_ids=('a',),max_resident_bytes=20,reveal_tick=0,deadline_tick=5,transition_model_root='t'); moved=InfluenceGraph(1,(('a','d'),)); d=validate_typed_closure_at_use(c,branch_id='A',support=s,influence=moved,transition_model_root='t'); self.assertEqual(d.disposition,TypedClosureDisposition.HOLD_INFLUENCE_IDENTITY)
 ...
 def test_support_identity_still_enforced(self):
  i,b,s,g=self.base(); c=compile_typed_closure(i,b,s,g,changed_evidence_item_ids=('a',),max_resident_bytes=20,reveal_tick=0,deadline_tick=5,transition_model_root='t'); moved=InvariantSupport(1,(('a','b','c'),)); d=validate_typed_closure_at_use(c,branch_id='A',support=moved,influence=g,transition_model_root='t'); self.assertEqual(d.disposition,TypedClosureDisposition.HOLD_SUPPORT_IDENTITY)
