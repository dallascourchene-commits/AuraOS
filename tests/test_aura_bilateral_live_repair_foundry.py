...
from aura_bilateral_live_repair_foundry import (
    ...
    BoundedRepairFoundry,
    build_current_reproof,
    build_preview_receipt,
    build_spatial_foundry_projection,
)
...
def replay():
    ...
    capture.mark_incident(
        "selection lost",
        {"api_key": "sk-secret-value-123456"},
    )
    return capture.finalize(
        expected_positive=["selection stable"],
...
def test_capture_redacts_binds_identity_and_grants_no_authority():
    packet = replay()
    encoded = json.dumps(dataclasses.asdict(packet))
...
def test_foundry_enforces_no_repeat_budget_and_all_proof_classes():
    ...
    second = foundry.record_candidate(
        hypothesis={"cause": "filter"},
    ...
    with pytest.raises(RuntimeError, match="budget"):
        foundry.record_candidate(
            hypothesis={"cause": "third"},
            candidate_digest=d("c3"),
            positive_passed=True,
            negative_passed=True,
            preservation_passed=True,
            adjacent_regressions_passed=True,
...
def test_preview_retains_verified_version_and_requires_rollback_reason():
    with pytest.raises(ValueError, match="rollback_reason"):
        build_preview_receipt(
            candidate_digest=d("c"),
            last_verified_digest=d("v"),
...
def test_current_reproof_requires_p0_p1_disposition_and_experience():
    packet = replay()
    incomplete = build_current_reproof(
        identity=identity(),
        ...
        p0_negative=True,
        p1_guardrail=False,
        p1_preservation=True,
        independent_verifier_id="p1",
        human_community_disposition="CONFIRMED",
        relationship_experience_ref="REL-1",
        qdkt_eligible=True,
    )
    assert not incomplete.durable_learning_authorized
    assert not incomplete.qdkt_eligible
    rejected = build_current_reproof(
        identity=identity(),
        replay_packet_digest=packet.packet_digest,
        candidate_digest=d("c"),
        p0_positive=True,
        ...
        p1_preservation=True,
        independent_verifier_id="p1",
        human_community_disposition="REJECTED",
        relationship_experience_ref="REL-1",
        qdkt_eligible=True,
    )
    assert not rejected.durable_learning_authorized
    assert not rejected.qdkt_eligible
    complete = build_current_reproof(
        identity=identity(),
        replay_packet_digest=packet.packet_digest,
        candidate_digest=d("c"),
        p0_positive=True,
    ...
    assert complete.durable_learning_authorized
    assert complete.qdkt_eligible


def test_spatial_projection_is_complete_but_non_authoritative():
    packet = replay()
    foundry = BoundedRepairFoundry(packet)
    attempt = foundry.record_candidate(
        hypothesis={"cause": "reset"},
        candidate_digest=d("c"),
        positive_passed=True,
        negative_passed=True,
    ...
    projection = build_spatial_foundry_projection(
        identity=identity(),
        incident=packet,
        attempts=[attempt],
        preview=preview,
        reproof=reproof,
        intent={"positive": ["keep selection"]},
        plan={"steps": ["repair"]},
