...
def test_compile_compass_combines_all_four_planes(monkeypatch, tmp_path: Path) -> None:
    ...
    assert packet["objective_contract"]["schema_version"] == (
        "AURA_CODING_RELATIONSHIP_COMPASS_OBJECTIVE_V1"
    )
    assert packet["objective_contract"]["safe_to_patch"] is False
    assert packet["connectome"]["capability_classes"]["ACTIVE"] == [
        "aura.relational.index"
    ]
    assert packet["connectome"]["zero_model_eligible"] is True
