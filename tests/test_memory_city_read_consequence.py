...
def t(receipt,reproof=('C',),transition='T'):return TypedClosureCertificate(TypedClosureDisposition.READY,None,hx('typed-'+receipt),hx('infl-'+receipt),reproof,0,transition,None,receipt)
...
class ReadConsequenceTests(unittest.TestCase):
    ...
    def test_currentness_move_holds_at_use(self):
        worlds,cov,cert=ready_cert(((h('rA','hA'),t('tA')),(h('rB','hB'),t('tB'))));d=validate_read_consequence_at_use(cert,hydration=worlds[0].hydration,fixed_support_root='rB',coverage=cov,current_program_root=hx('moved'),current_sealed_domain_root=cov.sealed_domain_root,current_coverage_generation=cov.generation);self.assertNotEqual(d.status,'READY_D0')
    def test_unknown_active_world_holds(self):
        worlds,cov,cert=ready_cert(((h('rA','hA'),t('tA')),(h('rB','hB'),t('tB'))));d=validate_read_consequence_at_use(cert,hydration=h('rC','hC'),fixed_support_root='rB',coverage=cov,current_program_root=cov.program_root,current_sealed_domain_root=cov.sealed_domain_root,current_coverage_generation=cov.generation);self.assertEqual(d.status,'HOLD_ACTIVE_WORLD_MEMBERSHIP')
    def test_mutation_never_authorized(self):
        worlds,cov,cert=ready_cert(((h('rA','hA'),t('tA')),(h('rB','hB'),t('tB'))));d=validate_read_consequence_at_use(cert,hydration=worlds[0].hydration,fixed_support_root='rB',coverage=cov,current_program_root=cov.program_root,current_sealed_domain_root=cov.sealed_domain_root,current_coverage_generation=cov.generation,mutation_requested=True);self.assertEqual(d.status,'HOLD_MUTATION_AUTHORITY');self.assertFalse(d.mutation_authority);self.assertFalse(d.effect_authority);self.assertFalse(d.gate10)
