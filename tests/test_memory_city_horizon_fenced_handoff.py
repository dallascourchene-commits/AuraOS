...
class ReadCert:
    status:str='READY_D0'; coverage_receipt_root:str=R('coverage'); program_root:str=R('program'); sealed_domain_root:str=R('domain'); coverage_generation:int=4
    binding_roots:tuple=(R('b1'),R('b2')); member_support_roots:tuple=(R('s1'),R('s2')); transition_model_root:str=R('t'); horizon:int=2
    future_congruence_root:str|None=R('f'); consequence_root:str=R('c'); receipt_root:str=R('receipt')

@dataclass(frozen=True)
class ReadUse:
    status:str='READY_D0'; reason:str='ok'; certificate_root:str=R('receipt')

@dataclass(frozen=True)
class TypedCert:
    disposition:str='READY_D0'; support_root:str=R('s'); influence_root:str=R('i'); transition_model_root:str=R('t'); horizon:int=2
    future_congruence_root:str|None=R('f'); receipt_root:str=R('tr')
...
def bundle(cert=None,use=None):
    c=cert or ReadCert(); u=use or ReadUse(); sem=semantic_handoff_root(c)
