...
@pytest.mark.parametrize(
    "reservation_id",
    [" owner-exhausted-2 ", "\towner-exhausted-2", "owner-exhausted-2\n"],
)
def test_exhausted_rejects_whitespace_padded_reservation_identity_without_effect(
    tmp_path: Path,
    reservation_id: str,
) -> None:
...
@pytest.mark.parametrize(
    "reservation_id",
    [" owner-lease-1 ", "\towner-lease-1", "owner-lease-1\n"],
)
def test_reserved_rejects_whitespace_padded_reservation_identity_without_effect(
    tmp_path: Path,
    reservation_id: str,
) -> None:
    _write_policy(tmp_path)
