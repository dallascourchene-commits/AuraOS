...
| Reasoning | Runs topology scans, neuro-symbolic omnipath sweeps, meta-resonance checks, coordinated Pass@K reasoning, Markovian workspace reconstruction, Aura-native meta-harness audits, and read-only emergent-potential audits over unwired module abilities. |
...
| Research ingestion | Forages arXiv, backtracks bounded chronological CS windows with legacy DB schema migration, keeps the REPL path metadata-first by default, preserves TeX/display/inline/bare mathematical formulas in paper-memory traces, parses opt-in PDFs, and gates synthesis through SkillWeaver. |
...
- **Backtrack reliability**: `!backtrack` now verifies and migrates the `traces` schema, wraps blocking arXiv reads in an outer async deadline, prints fetch/parse/ingest progress, defaults the REPL path to 25 metadata records with 0 PDF fetches, and preserves mathematical formulas in paper-memory traces.
...
| `!backtrack` stalled-window smoke, live arXiv | CS window `202605192202..202605202202` at offset 0 | Outer async deadline around `urllib` reads, metadata-first ingest, progress telemetry, formula-aware trace records | N/A | 8,555-byte Atom response; 3 rows ingested from 667 reported papers; 0 PDF fetches | 318.949 ms under a 30 s guard |
| Scientific memory regression tests | `test_scientific_memory.py` | Backtracker retry/fallback/default-PDF behavior plus formula metadata and trace coverage | N/A | 93 tests | 93 passed in 1.38 s |
| Emergent Potential REPL audit | Deterministic fake topology plus real checkout smoke | `AbilityAtom` and `EmergentConnection` report with JSON mode, proposed-function mode, no-network/no-write guards, and Architect bypass | N/A | Real smoke scanned 2,774 ability atoms and returned 5 candidates; `tests/` plus root routing/research checks stayed green | real smoke 3,961.563 ms; 71 tests passed in 1.46 s; 20 root checks passed in 11.81 s |
...
| RAEC paper memory | arXiv metadata and opt-in PDFs are chunked before VSA encoding, then lifted through a cached single-seed context profile, 10,000-D complex document vector, 1.2KB holographic header, three-point capsule, and formula metadata (`mathematical_formulas`, count, parser version). Egress scans the local JSONL ledger and injects only the top 2 bracket slots. |
...
| Emergent Potential audit | `emerge` / `emergent` / `future` / `potential` runs a read-only local audit over topology, CODEMAP-adjacent evidence, tests, ledgers, and manifests. It emits Markdown or JSON only, never writes code, never stages patches, never calls external APIs, and marks missing source spans as `NEEDS_GROUNDING`. Broad "find emergent/future potential" Architect intents are diverted to this route instead of Act Capsule patch mode. |
...
| arXiv backtracker resilience | The chronological arXiv crawler uses bounded one-day windows, paced requests, an outer async deadline for blocking reads, OAI-PMH fallback, configurable PDF fetch limits through `AURA_BACKTRACK_PDF_LIMIT`, formula-aware traces, and legacy `traces` schema repair before writing `Scientific VSA v1` rows. |
