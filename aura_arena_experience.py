"""Authoritative observable Arena experience records (V2)."""
from __future__ import annotations
from dataclasses import asdict, dataclass, field
import hashlib, json, math, re, secrets, time
from typing import Any, Mapping

ARENA_EXPERIENCE_VERSION = "AURA_ARENA_EXPERIENCE_V2"
...
class OutcomeVector:
    """Nullable observable dimensions; every projection is proposal-only."""
    terminal_class: str
    task_progress: float|None=None; evidence_quality: float|None=None
    verification_quality: float|None=None; safety_quality: float|None=None
    human_alignment: float|None=None; cost_efficiency: float|None=None
    latency_efficiency: float|None=None; abstention_quality: float|None=None
    recovery_quality: float|None=None
    measurement_classes: dict[str,str]=field(default_factory=dict)
    labels: tuple[str,...]=()
    version: str=OUTCOME_VECTOR_VERSION; proposal_only: bool=True
    patch_authority: str=PATCH_AUTHORITY; vsa_patch_authority: bool=False
    ...
    def from_dict(cls,value:Mapping[str,Any]):
        ...
        data["labels"]=tuple(str(x) for x in data.get("labels",()) if str(x)); data["measurement_classes"]={str(k):str(v) for k,v in dict(data.get("measurement_classes") or {}).items()}
        return cls(**data)
    def to_dict(self):
        data=asdict(self); data["labels"]=list(self.labels); return data
    def proposal_projection(self,weights:Mapping[str,float]|None=None):
...
class ArenaExperience:
    experience_id:str; correlation_id:str; task_id:str; workflow_id:str
    arena_id:str; arena_version:str; grammar_version:str; grammar_manifest_digest:str
    runtime_version:str; compiler_version:str; started_at:float; completed_at:float
    state_before:str; state_after:str; selected_transition:str; final_outcome:str
    outcome_vector:OutcomeVector
    admissible_alternatives:tuple[dict[str,Any],...]=(); predictions:tuple[dict[str,Any],...]=()
    route_observation_digest:str=""; repository_commit_sha:str=""; working_tree_digest:str=""
    objective_hash:str=""; source_hash_digest:str=""; provider:str=""; model:str=""
    measurement_class:str="UNAVAILABLE"; cost_run_id:str=""; trace_atom_ids:tuple[str,...]=()
    raw_evidence_refs:tuple[str,...]=(); payload:dict[str,Any]=field(default_factory=dict); redactions:tuple[str,...]=()
    version:str=ARENA_EXPERIENCE_VERSION; patch_authority:str=PATCH_AUTHORITY; vsa_patch_authority:bool=False
    learned_weight_patch_authority:bool=False; crystallization_patch_authority:bool=False
    def to_dict(self):
...
def build_arena_experience(*,arena_id:str,arena_version:str,grammar_version:str,runtime_version:str,compiler_version:str,state_before:str,state_after:str,selected_transition:str,final_outcome:str,payload:dict[str,Any]|None=None,grammar_manifest_digest:str="",outcome_vector:OutcomeVector|Mapping[str,Any]|None=None,admissible_alternatives=None,predictions=None,experience_id:str="",correlation_id:str="",task_id:str="",workflow_id:str="",started_at:float|None=None,completed_at:float|None=None,repository_commit_sha:str="",working_tree_digest:str="",objective:str="",source_hashes=(),provider:str="",model:str="",measurement_class:str="UNAVAILABLE",cost_run_id:str="",trace_atom_ids=(),raw_evidence_refs=()):
    safe_payload,reds=sanitize_experience_payload(dict(payload or {})); route=_route_packet(safe_payload)
    manifest=str(grammar_manifest_digest or route.get("grammar_digest") or "").strip()
    if not manifest: raise ValueError("grammar_manifest_digest is required for every ArenaExperience")
    derived_alt,derived_pred=capture_route_observation(route)
    safe_alt,r1=sanitize_experience_payload(derived_alt if admissible_alternatives is None else list(admissible_alternatives))
    safe_pred,r2=sanitize_experience_payload(derived_pred if predictions is None else list(predictions))
...
def capture_route_observation(route:Mapping[str,Any]|None):
    ...
    for pos,row in enumerate(alternatives):
        ...
        predictions.append({"transition_id":str(row.get("transition_id") or ""),"rank_position":pos,"predicted_selected":str(row.get("transition_id") or "")==selected,"predicted_next_state":str(row.get("next_state") or ""),"semantic_fit":row.get("semantic_fit"),"rank":rank,"measurement_classes":dict(rank.get("measurement_classes") or {}),"risk":str(row.get("risk") or "unknown"),"required_evidence":list(row.get("required_evidence") or []),"produced_evidence":list(row.get("produced_evidence") or []),"meta_transition":bool(row.get("meta_transition"))})
    return alternatives,predictions

def derive_outcome_vector(*,final_outcome:str,payload:Mapping[str,Any]|None,state_before:str,state_after:str):
    data=dict(payload or {}); route=_route_packet(data); selected=dict(route.get("selected") or {}); result=data.get("action_result") if isinstance(data.get("action_result"),dict) else {}; terminal=str(final_outcome or "UNKNOWN").upper()
    success={"ALLOWED","COMPLETED","PASS","PASSED","SUCCEEDED","SUCCESS","VERIFIED","META_COMPLETED"}; failure={"DENIED","FAILED","FAIL","ERROR","INVALIDATED"}; abstain={"BLOCKED","ABSTAINED"}
    progress=1. if terminal in success else (0. if terminal in failure|abstain else None)
    required={str(x) for x in selected.get("required_evidence",[]) if str(x)}; keys={str(x) for x in data.get("evidence_keys",[]) if str(x)}; missing={str(x) for x in result.get("missing_evidence",[]) if str(x)}
    evidence=max(0.,min(1.,(len(required&keys) if keys else max(0,len(required)-len(missing)))/len(required))) if required else (0. if missing else (1. if selected else None))
    verifier=str(selected.get("verifier_requirement") or "none").lower(); verification=1. if terminal in {"VERIFIED","PASS","PASSED"} or result.get("verification_ok") is True else (0. if verifier!="none" and terminal in failure else None)
    violation=any(bool(data.get(k) or result.get(k)) for k in ("active_grammar_mutated","automatic_commit","automatic_push","automatic_merge","learned_weight_patch_authority","crystallization_patch_authority")); safety=0. if violation else 1.
    approval=str(selected.get("approval_requirement") or "none").lower(); human=(1. if terminal in success else 0.) if approval not in {"","none"} else None
    rank=dict(selected.get("rank") or {}); classes=dict(rank.get("measurement_classes") or {})
...
def sanitize_experience_payload(value:Any):
    red=[]
    def walk(item,path=""):
        ...
        if isinstance(item,(list,tuple,set)):
            seq=sorted(item,key=lambda x:(type(x).__name__,str(x))) if isinstance(item,set) else item
            return [walk(x,f"{path}[{i}]") for i,x in enumerate(seq)]
